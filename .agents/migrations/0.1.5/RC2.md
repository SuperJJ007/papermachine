...
范围：把官方 `dsh-v0.1.5-rc.2`（`fb2c4b9e698e30edb738bca4cf0618587db7d203`）merge-forward 到 P0 基线 `7545be293debb9623e439e2257383bf4b39cf3da`。基线已包含 rc.1（合并提交 `d2409f7235` 的第二父），因此本次是 rc.1→rc.2 的增量合并，不是快进。
...
- 本阶段代码变化（`7545be293d..0888810bd4`）：343 个文件，+1059/−1059；后续交付提交在外部交接报告记录。
...
下表保留前轮交付方报告的检查结果，不代表本轮重跑；代码候选为 `0888810bd4`。本轮文档检查结果见外部交接报告。
...
当前增量以 rc.2 为基准度量：`fb2c4b9e69..0888810bd4` 为 1508 个文件、+129876/−2596（`--no-renames`），不包含其后的交付文档和证据。以 rc.1 为基准会把 rc.2 自身的发版算作私有工作，两者相差 322 个文件。

历史 rc.1 台账保持其历史含义不改写。相对 rc.2，52 行中 31 行与历史数字一致、21 行漂移；历史切片合计 +2560/−737，当前增量合计 +4957/−718。缺口分两类记录在 `evidence/RC2/`：228 个 rc.2 已存在文件带私有修改但无台账行（+5978/−1878），1034 个文件在 rc.2 不存在（+118941/−0，不存在本身不能确定作者归属，升级规划前需审查）。详见 [rc.2 私有补丁台账](../../notes/implemented/process/2026-09-11-replant-rc2-patch-ledger.md)。
