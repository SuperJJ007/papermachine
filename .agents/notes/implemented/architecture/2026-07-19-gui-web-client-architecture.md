...
- The snapshot store engine **lives in the runtime package** (zustand vanilla with draft-based updates, `flush: 'sync'` by default with opt-in `'raf'` batching, opt-in localStorage persistence that merges a rehydrated payload's top-level keys over `init()` and excludes any `transient`-declared field ([rehydration merge](../../proposed/architecture/2026-08-31-client-store-rehydration.md)), dev-mode deep freeze — all exported from `runtime`'s `./client` main entry, no subpath): store products are bare observable sources with no hook members. Plugins reach the engine only through `defineStore` declarations per the [slot system standard](2026-07-22-slot-type-chain-implementation.md). ui-renderer composes every hook at the binding site (`bindSnapshotSelector`, per-source cached) from the one data contract React consumes: `ObservableSnapshot<T>` (`getSnapshot`/`subscribe`) — a Session object and a snapshot store both satisfy it.
...
src/client/
  ...
  input/       composer UI and state machine
...
Public data and callback declarations live in `contract/`, independently of their implementing classes. The pending-interaction face exposes response delivery; Session alone settles waits. Shared object mechanisms such as scope tags and notification batching live at the runtime package root. Package-private pure glyphs, formatting, and transcript readers may likewise serve several presentation domains without owning a service, store, or registration. A component and its private children remain one domain; the composer's controls belong to `input/`, and workspace browser rows belong to `browser/`.
