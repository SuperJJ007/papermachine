...
`verify-cordis-config`'s source-plane check scans `pluginReferences` collected from yml rows, so a package that is only ever mounted by a runtime string — not a bare plugin `name:` row — stays invisible to it even when `tsconfig.base.json` has no mapping for it. `packages/host/directory-picker-auto` names its four backend packages this way (`CHOOSER_BACKEND_PACKAGES`), so `tsconfig.base.json` shipped without entries for `dsh-client-ui-directory-picker-native`/`-browse` and the tsx source launch of the desktop Host died on `Cannot find package` the first time the chooser mounted them, on an otherwise-clean checkout with no built `lib/` to fall back to. The source-plane check now also requires every `CHOOSER_BACKEND_PACKAGES` entry whenever its mounting chooser package appears in a row, mirroring the check's existing `missingPluginDependencies` treatment of the same set; a package added to that runtime-string list without a matching `tsconfig.base.json` mapping now fails the gate instead of only the next desktop launch.
