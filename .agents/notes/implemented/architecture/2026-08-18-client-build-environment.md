...
Browser tests that run watch builds own restoration of both dynamic plugin bundles and the complete Vite output directory, including removal of newly emitted files. They stop all writers before restoring the saved artifacts and validate the original build record afterward; recording a new digest would hide test contamination.
