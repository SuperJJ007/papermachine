...
**Artifact 与 Version 记录**(spec 第 2 条)。本段描述的是本 note 当初上线的 v1 schema:Artifact 行 `artifactId`(branded、生成)、`owningProjectId`、`originSessionId`、`logicalName`、`latestVersionId`、`createdAt`;Version 行 `versionId`、`artifactId`、artifact 内连续的 `ordinal`、`parentVersionId`(可空;可指向另一 artifact 的版本,保留今天 `editBaselines` 的跨 artifact 派生)、`sha256`、`mediaType`、`byteCount`、`origin`(`auto` | `model` | `human-edit`)、`title`/`caption`、producer 溯源、`createdAt`,策展(`annotate_artifact`)是原地元数据更新。这份记录形态及其 `origin`/`parentVersionId`/策展规则已被 [schema v2 迁移](2026-09-01-project-artifact-store-schema-v2.zh.md) 取代:`content_origin`/`AnnotationActor` 拆分为两个字段,`base_version_id`/`base_explicit` 取代了原先默认推导的 `parentVersionId`,元数据编辑改为追加一行 `version_annotations` 而非原地更新版本行。本 note 下文规定的 store 目录布局、project 身份、线性化与删除边界不受影响,仍是现状。

**Session 日志与 store 的分工。** model-visible ⟺ logged 规则在双权威下的重述:session 日志记录*事件与引用*,store 拥有*字节与跨 session 索引*。本 note 当初上线的 v1 事件字段列表——`science/artifact-saved` 从内嵌附件瘦身为 `{artifactId, versionId, ordinal, sha256, mediaType, origin, parent, title, caption}`——已被 [store 权威规则](2026-09-01-project-artifact-store-schema-v2.zh.md) 及随后的 [事件瘦身](2026-09-02-science-artifact-event-slimming.zh.md) 取代:事件现在只携带 `{artifactId, versionId, version, logicalName, sha256, title, caption?, projectId, seenAt}`,`contentOrigin`、producer、base version、`createdAt` 等每一个溯源事实都只活在 store 里。本段描述的跨 session 信任机制(该 session 自身 fold 完全没有本地记录的引用会被信任)不受这次瘦身影响,仍是现状——见 S3 note,其机制被 [runtime provenance-writes note](2026-09-02-science-runtime-provenance-writes.zh.md) 恢复并延伸。跨 project 引用仍完全在范围之外,遵循上文 v1 spec 条目的延后决定。Session 导出的附件抽取按校验和改指向 project store。
