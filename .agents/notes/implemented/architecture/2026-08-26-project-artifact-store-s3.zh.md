...
**严格 fold 会信任一个自己无法独立验证的跨 session 引用,而不是拒绝它。** `science-session/src/transition.ts` 中原有两处检查都假定每一个 artifact 引用都能从该 session 自身累积的 fold 状态中解析出来——这个假设在 S3 之前始终成立,因为在那之前每一个引用本来就确实如此:
- `applyArtifactSaved` 的连续性检查不再要求该 session 从未记录过的逻辑名必须从版本 1 开始。它既可能是一个真正全新的 artifact,也可能是在接续另一个 session 已经在同一 project 中开出的版本——该 session 的 fold 只重放自己的 log,无法在不借助 store 的情况下分辨这两种情况,因此它接受 codec 自身 `POSITIVE_INTEGER` schema 已经许可的任意 ordinal。同一 session 内的接续(该 session 已经记录过的逻辑名)仍然要求严格连续:这种情况仅凭 log 就能验证,也确实在验证。
- `applyRunStarted` 的 run input 检查(`requireRunInputArtifactVersion`,替换了这一处原本共用的 `requireArtifactVersion`)现在在所引用的 artifactId 根本不在 `state.artifacts` 中时会直接返回,不再抛错。若该 session 的 fold 已经记录过某个 artifactId,但指名的版本号不对,仍会抛错——这一同 session 内的不一致,仅凭 log 就能识破。
...
`science-runtime` 的自动捕获遍历与 `prepareRunArtifacts` 现在除了 S2 已经使用的读写方法外,还依赖 `ScienceArtifactStore.listArtifacts`/`listVersions`;store 没有任何公开方法签名发生变化。`science-session` 的严格 fold 现在只接受一个更窄的、此前会被拒绝的 `science/run-started`/`science/artifact-saved` 值子集——同一 session 内版本号错误的引用,或同一 session 内的连续性跳跃,仍然照旧被拒绝;只有该 session 自身的 fold 完全没有记录过的引用才会被新接受。`packages/science/science-session/tests/fold.spec.ts` 与 `invariant.spec.ts` 相应更新了 fixture(一个"本地已知但版本号错误"的用例,取代了原来那个"artifactId 未知"、如今已不再抛错的用例)。
...
以下为本切片之前已存在、未被本切片触碰的事实:`packages/science/science-session/src/applicability.ts` 中 presetId 自洽性分支(由 S2 的 presetId rider 新增)低于按文件 100% 的覆盖率 gate——S2 自己的交接文档已经指出该 rider 没有跑过覆盖率;本切片自己改动的文件(`transition.ts`、`capture.ts`、`inputs.ts`)均为 100%。
