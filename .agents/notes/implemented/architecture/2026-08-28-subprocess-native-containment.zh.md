...
request 被消费或 manager 已观察到 loaded 且非终态的 unit，都能建立 scope ownership。在这两项事实出现前，只要 direct launcher 仍在运行，unit absence 或 loaded inactive/failed 元数据就保持未决：元数据可能在启动任务激活 scope 前出现。过早宣称完全停稳会取消强制终止，而 launcher 此时仍可能启动 target。如果 launcher 退出时 request 仍未消费，已观察到的终止信号仍作为 direct result；无信号退出则以 startup failure reject。此区别允许取消尚未启动完成的 bootstrap，而不误报提供方故障。range observation 独立确认 launcher 已退出且 manager 报告无 unit 后，成功结算 empty-range wait。parent 每 50 毫秒检查一次这段未决区间；建立后，状态查询按指数增长间隔退避，最多达到既有的 5 秒 systemctl 上限。每次查询同时读取 `LoadState` 与 `ActiveState`：已经建立或 launcher 已退出后的 loaded `inactive` 或 `failed`，以及已经建立的 unit 变为 `not-found`/`inactive` 或被 collect 卸载，都能证明 range 为空。`active`、`activating`、`reloading` 与 `deactivating` 仍是非终态。未知或 malformed 组合以及不可读的 manager 结果会使 `waitForExit()` reject，而不是宣称完全停稳。`terminate()` 会唤醒正在休眠的 observer 立即复查，结算时会取消未胜出的退避 sleep。scope `SIGKILL` 成功后，owner 还会以非阻塞方式请求 unit 停用：在 bootstrap 消费 request 前终止 launcher，可能留下空但仍 active 的 scope。除非观察结果证明 range 为空，否则 stop 失败仍作为终止失败保留；stop 请求本身不证明完全停稳。严格的同目录 `startup-error.json` 只承载 request／bootstrap 或 target pre-exec failure，parent 会在可观察生命周期完成时移除本次 spawn 的私有路径。
