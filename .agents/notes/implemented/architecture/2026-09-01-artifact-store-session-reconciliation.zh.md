...
**`reconcileMaxVersions`（store 包，默认 2000）与 `reconcileMaxSessions`（runtime 包，默认 500）是两个独立的上限，因为它们限定的是两个不同包各自负责的两种工作量。** store 限定的是它自己的 version 行加悬空事件，一次 `reconcileProject` 调用处理多少（分批，超出报告 `truncated: true`）；runtime 限定的是一个 project 的对账触发点读多少条 session 日志才归并事件（`collectProjectArtifactEvents` 自己的 `truncated`）。两个上限都让每次 fire-and-forget 尝试保持有限；两者都不会创建后台任务或重试循环。

**不完整的 session 事件集绝不会把缺少事件转成 orphan 健康状态。** session 列举失败、任一匹配日志不可读、任一目标事件格式错误，或 session 截断都会让 `CollectProjectArtifactEventsResult.complete` 变成 false。此时，`reconcileProject` 把没有匹配事件的存储行分类为 `unverified`，并在健康状态 patch 中省略 orphan 字段，保留已经记录的任一值，因为部分读取既无法证明它，也无法排除它。内容冲突、元数据比较、悬空事件重建和内容缺失刷新仍会根据已有证据执行。只有完整事件收集以及未截断、无错误的 store 对账运行才会进入 `reconciledProjects`；其他结果仍可在后续 project 解析发生时重试，且距上次尝试至少经过已校验的 `reconcileRetryDelayMs`（默认 1000）。这种由触发点驱动的重试既避免永久误报，也不会形成自主热循环。
...
T3b 的 `dsh-api-proxy`／`ui-science` 透传所需公开读接口已经就位：`ScienceArtifactStore.getReconciliationSummary(projectId): Promise<ReconciliationSummary>`（`{orphanCount, reconstructedCount, missingContentCount, items}`，对上一次记录的 `version_health` 状态做纯读取）与 `ScienceArtifactStore.reconcileProject(projectId, events, eventSetComplete): Promise<ReconcileResult>`（写入口方法，由 `dsh-science-runtime` 调用，不由 BFF 直接调用）。T3b 自己的范围，即 `scienceLibrary` 响应的 `health` 字段、Files 面板提示条，以及“`orphan` 从不在 UI 中出现”规则，除了消费概述读接口外，不受本 Note 的其他决定影响。
