...
**只有当某个 session 自己的 `SessionPersistenceRevision` 仍与记录值一致时，缓存的逐 session 结果才可信。** `collectProjectArtifactEvents` 通过 `SessionPersistence.listSnapshots()` 列举 session，其逐 session `revision` 读取代价很低（JSONL 后端只需一次 `stat()`，SQLite 后端只需一次索引行读取），无需解析完整日志。`CollectProjectArtifactEventsCursor.revisionsBySession` 记录每条缓存的 `eventsBySession` 项被读取时观察到的 revision；一旦某 session 的实时 revision 不再匹配该记录值，就会把它从缓存中剔除并在同一次调用中重新排队检查，而不是信任这条陈旧缓存。如果没有这一机制，两次尝试之间被追加过的 session——例如，在更早一次尝试对另一 session 的检查仍未完成时——会永远保留它第一次、如今已不完整的 `eventsBySession` 项，导致后续一次完整对账把 store 中如今合法的新版本错误分类为 `orphan`，而不是把它与该次追加实际产生的事件匹配起来。
