...
**影响**：`packages/client/ui-science/src/client/version-summaries.ts`（新增）、`science-artifact-url-loader.ts`（新增）、`library-artifact.ts`（两个重新声明的类型）、`index.ts`（每处注册的 loader 接线，加上 `loadVersions`/`saveArtifactAs` 注入）、`ScienceDetailsView.tsx`/`ArtifactContent.tsx`/`ScienceOutcomeRow.tsx`/`ScienceTraceView.tsx`/`science-trace-model.ts`/`ScienceArtifactProvenance.tsx`（每一处消费被删字段的渲染期代码，改用 `ScienceRenderableVersion`）及其测试。这次瘦身原本打断的三项能力当时都没有客户端可用的读路径，只在各自确切的调用点记录而不做绕过：实时图表编辑面板、四页溯源下钻，以及 C2 同轮次中间稿折叠。图表编辑面板已经通过[客户端图表状态读路径](2026-09-02-science-client-chart-state-read-path.zh.md)恢复；确切的生产者事实现在也恢复了[artifact 溯源下钻](../bug-fix/2026-09-02-science-artifact-provenance-restoration.zh.md)，同时没有把生产者字段放回 Session 投影。C2 折叠仍然没了。`packages/client/connection` 的 fixture 与 `FakeApiClient` 测试替身也需要这次 wire 拓宽带来的 `health` 字段与 `scienceVersions`/`scienceArtifact` 桩，和上面的都无关，只是被同一道 tsc 门禁一并抓出来的。
