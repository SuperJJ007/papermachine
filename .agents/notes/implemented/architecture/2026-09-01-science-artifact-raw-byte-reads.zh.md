...
**影响**:`packages/client/ui-science/src/client/version-summaries.ts`(新增)、`science-artifact-url-loader.ts`(新增)、`library-artifact.ts`(两个重新声明的类型)、`index.ts`(每处注册的 loader 接线,加上 `loadVersions`/`saveArtifactAs` 注入)、`ScienceDetailsView.tsx`/`ArtifactContent.tsx`/`ScienceOutcomeRow.tsx`/`ScienceTraceView.tsx`/`science-trace-model.ts`/`ScienceArtifactProvenance.tsx`(每一处消费被删字段的渲染期代码,改用 `ScienceRenderableVersion`)及其测试。这次瘦身砸掉的两项能力没有客户端可用的读路径能补回来,只在各自确切的调用点记录清楚,不做绕过:实时图表编辑面板(`ArtifactContent.tsx` 自己的 JSDoc——没有读路径能提供 `ScienceChartState`,`previewChartOps` 又拒绝空的 ops 列表)与 C2 同轮次覆盖草稿折叠(`intermediate-versions.ts`,连同其测试一起删除——需要 `origin`/`producerSessionId`/`turn`,这些在客户端安全类型里全没了)。`packages/client/connection` 的 fixture 与 `FakeApiClient` 测试替身也需要这次 wire 拓宽带来的 `health` 字段与 `scienceVersions`/`scienceArtifact` 桩,和上面的都无关,只是被同一道 tsc 门禁一并抓出来的。
