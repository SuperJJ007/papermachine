...
The native Sidebar persists per-session layouts and identity counters, validates the complete stored graph, and adopts restored resource occurrences synchronously. Undo history is page-local: persisting executable layout operations would require an independent operation-log validator without adding a product control. The layout package persists rightbar width. Store persistence merges saved fields with initialized defaults and excludes declared transient fields. Each artifact pane owns provenance and lightbox state independently; Process disclosures survive sibling-view switches within a session.

Process and Trajectory are sibling conversation views. Science uses public input dock, header utilities, turn-tail, tool-view and tab-menu registrations plus the native Guide entry. A blank session has an identity before its first message; without a session no rightbar action is rendered. Native Turn Process Folding, context visibility and turn metrics remain in charge of Chat presentation.
...
The library opens through the session header, artifact menu and native Guide; the left navigation has no library action. The native Guide card supplies a library icon and description aligned with Workspace files. Blank sessions retain the header corner seat so the native expand control can reopen the sidebar without displaying the full conversation header.
