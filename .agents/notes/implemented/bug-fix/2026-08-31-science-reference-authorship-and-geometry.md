...
A blocked reference `+` control (a staged direct edit not yet committed or discarded, or an unsaved preview displayed) carries dimmed styling and a `title` tooltip, and the panel's notice paragraph names the exact reason instead of the default reference-selection help text. This is presentation only: the panel still blocks new element and region references during both states, on which `ScienceChartEditPanel.tsx`'s design already relies (per this note's Decision); whether to relax that exclusion — for example, letting reference selection continue during a staged direct edit — remains an open product decision.
