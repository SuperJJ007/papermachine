...
When `annotateArtifact` cannot resolve a logical name from the session projection, it checks retained run artifact directories newest-first with the capture walk's existing safe-path rules. Finding the exact eligible PNG changes only the `ARTIFACT_NOT_FOUND` message: it directs the caller to rerun the writing code with the path declared, then annotate again. This diagnostic reads path metadata only. It never reads or imports the file bytes, mutates the project artifact store, or changes annotation's metadata-only meaning. A failed or unavailable diagnostic walk falls back to the ordinary unknown-artifact error.
