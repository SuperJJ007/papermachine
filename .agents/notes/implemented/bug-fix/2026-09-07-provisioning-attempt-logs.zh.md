...
中间某个源的失败也不再是运行过程中不可见的了：下一个源的 `phase: 'solving'`"Retrying via …"进度消息现在会追加上一次失败的首行错误信息和它那份日志的完整路径（双语，见 `buildRetryMessage`），这样在 TUNA→USTC 换源过程中盯着进度面板看的用户，能看到运行为什么继续往下走、以及去哪里找那次失败尝试的完整输出，而不只是"它确实换源了"这一件事。

`onboarding.ts` 的诊断报告按钮从捕获到的错误文本里解析出 `Attempt logs:` 标记（`attemptLogPaths`，对已经收到的 message 字符串做一个小正则匹配——鉴于上面 Electron 剥属性的事实，这是唯一可用的信道），并在现有的 `- Error:` 围栏代码块之后追加一段 `完整日志文件 · Full log files:`，列出每一条路径（当这次失败确实带了任何日志路径的时候；一次容量或平台不支持之类、根本没走到源循环的失败不会带任何路径，报告在那种情况下不变）。`stripIpcErrorPrefix`（为 issue #22 的 app-local-CRT 修复而加）未受影响——这一段是在那次剥离已经发生*之后*追加的，不是之前。
