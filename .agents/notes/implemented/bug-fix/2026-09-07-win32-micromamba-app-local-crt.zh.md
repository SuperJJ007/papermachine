...
本次改动自己的自动化流程在这次会话里被真实跑过，不只是单测：`pnpm --filter @deepseek-ai/dsh-desktop run fetch:micromamba win32-x64` 针对真实的锁定 URL 真的跑了一遍，从真实的 `vc14_runtime` 包里下载并校验了真实的 `micromamba.exe` 和全部 11 个真实 DLL 到 `resources/bin/win32-x64/`（文件大小与包自身根目录列表逐一对上，这一点是在本应用代码之外，直接用 `unzip`/`tar`/`zstd` 检查下载下来的 `.conda` 文件核实的）。一次完整的 Windows 打包流程（`pnpm -w run build --profile official && pnpm run build && pnpm run fetch:micromamba win32-x64 && pnpm run stage:host && pnpm exec electron-builder --win --publish never -c.electronDist=<本机缓存的 electron-v43.4.1-win32-x64.zip> && node --import tsx/esm scripts/write-update-metadata.ts .exe`）确认了 `electron-builder.yml` 平台专属 `extraResources` 的产出布局：`release/win-unpacked/resources/bin/` 下只有 `win32-x64/`，里面是 `micromamba.exe` 加全部 11 个 DLL。打出的安装包 `release/PaperMachine-0.1.0-x64.exe` 大小 156,778,225 字节，SHA-256 为 `ee942a0346ec7577dcff115c85acf0a42f0bb3861d4179c8aa60666f3e94d495`，与 `release/desktop-update.win32.json` 一致。同时跑的一次 `mac --dir --arm64` 构建确认了 mac 侧同样的布局：`release/mac-arm64/PaperMachine.app/Contents/Resources/bin/` 下只有 `darwin-arm64/`，里面只有 `micromamba` 一个文件，没有混入 win32 或 darwin-x64 的东西。
