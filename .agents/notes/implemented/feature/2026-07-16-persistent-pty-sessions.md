...
PowerShell startup submits its setup command once and waits for `stdin_read` across send completions. The published `motd` reads the bounded session scrollback: an output-free completion can establish readiness without replacing earlier startup output.
