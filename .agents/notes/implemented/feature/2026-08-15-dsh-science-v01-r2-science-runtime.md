...
At the time, the accepted DSH Science v0.1 line had the official RC5 release baseline and the R1 Science Session domain, but no producer for `science/environment-bound`, `science/run-started`, or `science/run-finished`. The downstream Runtime at `omdsh-dev/dsh-science@e5e8b29b435f67e0a5dde5e2132580966e78b27b` was built with four generic subprocess and sandbox extensions that RC5 does not contain. Its implementation commits also mix package code, existing-Consumer migrations, generated documentation, and unrelated repository repairs. Copying the downstream branch or cherry-picking those commits would therefore import work outside the Runtime's ownership.
...
Binding, run setup, and artifact annotation share one exact-Session reservation. `bindEnvironment`/`annotateArtifact` reserve it non-queuing: a second such operation on the same live Session returns `RUNTIME_BUSY` immediately. `startRun` instead queues behind whichever operation already holds it (`reserveQueued`, the [run-queueing decision](../bug-fix/2026-08-25-science-runtime-queue-concurrent-runs.md)), so a genuinely concurrent second run gets a real turn once the first vacates instead of an outright rejection. A detached lifecycle retains a same-ID quarantine until every owned probe and process tree is quiescent and cleanup has settled. Each append rechecks `ctx.sessions.get(session.id) === session`, so an old object cannot write into a same-ID successor.

Environment binding observes and confines the interpreter before appending one whole `science/environment-bound` value. A run's execution mechanism — a persistent per-(session, language) kernel process rather than a fresh spawn per run — is owned by the [persistent-kernel decision](../architecture/2026-08-20-science-persistent-kernel.md), which also carries the kernel lifecycle fact and the run→epoch identity R2 did not yet have. A detached Session receives no terminal append; replay derives `interrupted` from the unmatched durable start. Failure to prove quiescence or commit a terminal fact never returns a value that looks durably settled.

Every probe uses direct argv, `environmentBase: 'empty'`, an owned cwd, fixed locale/timezone, and full `workspace-write` confinement; Python probes use the frozen isolated UTF-8 flags, and R version discovery uses standalone `Rscript --version` with its UTF-8 probe under `--vanilla --encoding=UTF-8`. A kernel process is confined the same way, held for its whole lifetime rather than freshly per run (see the persistent-kernel decision linked above). Scratch lives only below the resolved DSH home, uses exclusive owner markers and private modes, rejects symlinks and path overlap, and retains accepted run state while removing only unpublished setup that the current operation owns.
...
This Note does not supersede the Science Session, subprocess, sandbox, session-log, timeout, home-path, invariant, or distribution decisions. It consumes or narrowly extends them. It links back to those owners and states only the behavior that landed. The [persistent-kernel decision](../architecture/2026-08-20-science-persistent-kernel.md) partially supersedes this Note: it owns the run execution mechanism, replacing the fresh-spawn-per-run description above, while every other R2 result — provenance, scope, generic-capability additions, and confinement/quarantine mechanics — remains this Note's own.
