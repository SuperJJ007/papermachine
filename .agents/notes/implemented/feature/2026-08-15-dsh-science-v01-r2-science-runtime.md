...
Real Python and R acceptance remain opt-in and host-dependent. An environment or host failure is literal FAIL/`NOT-RUN` until a successful rerun on the recorded candidate; historical downstream PASS does not close R2. The next implementation is the model-facing Science tool Consumer, and only after Runtime Context and filesystem read-only work that this Note still excludes.
