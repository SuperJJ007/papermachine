...
The plugin statically injects only `tools` and `systemPrompt`. It reads the optional Host-owned Runtime with `ctx.get('scienceRuntime')` at the earliest operation that needs it — first-use binding, and each `run_python`/`run_r` call. Model-facing operations require an exact initiating Agent whose Session currently resolves to the `science` preset identity (`isScienceSession`, `@deepseek-ai/dsh-agent-presets`'s `resolveSessionPreset` over the creation header and any later `agent-preset/selected` event — see the [Science product gaps Agent Note](../bug-fix/2026-08-18-science-mode-product-gaps.md) for why the header alone is insufficient). Diagnostic prompt assembly without an Agent and turn signal performs no Host I/O and delegates unchanged.
...
A static `tool:science` prompt section states persistence and kernel-restart semantics for each language — owned by the [persistent-kernel decision](../architecture/2026-08-20-science-persistent-kernel.md), which also owns the roles of `SCIENCE_STATE_DIR`, `SCIENCE_ARTIFACT_DIR`, and `SCIENCE_WORKSPACE_DIR` — plus that the current directory is the per-run output directory, workspace files require the environment variable or an absolute path, terminal program failure is a result to inspect, and infrastructure failure means no trustworthy run result exists. The deterministic `science:environment` dynamic context contains only durable mode and environment identities/status, interpreter capability/version and a truncated fingerprint, the file-based state rule, and a bounded latest-run summary. It omits Runtime-owned free-text reasons, source, stdout, stderr, credentials, and Host path/identity fields, and renders `''` outside Science mode or without an initiating Agent.
