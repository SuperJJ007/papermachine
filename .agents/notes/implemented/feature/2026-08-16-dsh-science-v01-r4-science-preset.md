...
| Accepted R4 candidate | `ac57329b7a2a6912734dee84774ea67b84859007`, seven linear commits above the plan base | Exact accepted source behavior for the built-in preset and its copyability prerequisite |
...
| IN | Copy-safe preset authoring | A generic, explicit preset-copyability field that defaults to copyable; the built-in `science` metadata disables copying, `ctx.agentPresets.copy()` and `agentPreset.copy` reject it, and Web exposes no actionable copy path |
...
R4 source evidence includes the copyability/API/client unit suite (372 tests, 17 files), the CLI e2e composition file (35 tests), the two Web browser lanes plus the new Web snapshot (14 tests), the built-artifact lib-mode snapshot re-run, `typecheck`, `build`, `check:ci:artifacts` (5/5), `verify-cordis-config`, `doc-sync` (28/28), `verify-agent-note-format`, and `lint`. Two hygiene sub-checks — `rescope-vendor:check` and `knip` — fail identically to the R4 plan base and are disclosed as pre-existing rather than reported as passing; the pre-existing 8-clone `duplication` gap likewise carries no R4-added file. An independent clean-context review of the full range returned no `BLOCK` or `HIGH` finding; the one closed nit (a stale generated line-number cross-reference disturbed by the `AgentPreset.copyable` regeneration) is repaired in the accepted candidate. The [dated R4 evidence record](../../../../docs/evidence/2026-08-16-dsh-science-v01-r4-science-preset.md) binds every result and exception to the accepted candidate SHA.
