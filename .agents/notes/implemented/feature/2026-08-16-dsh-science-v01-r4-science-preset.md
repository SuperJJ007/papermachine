...
At R4, the exact model tool roster was `ask_user_question`, `get_science_state`, `glob`, `grep`, `read`, `read_image`, `run_python`, `run_r`, `skill`, and `todo_write`, excluding bash/pwsh, filesystem write/edit, `str_replace_editor`, jobs, Goal, plan mode, subagents, workflows, Ralph, Code Mode, self-modification, Web search, and chart/Outcome publication. `annotate_artifact`, `install_science_packages`, `exit_plan_mode`, `web_search`, and `web_fetch` joined it later ([R5 charts/Outcome](2026-08-16-dsh-science-v01-r5-charts-outcome.md), [package installs](2026-09-01-science-package-install.md), [web and plan](2026-09-01-science-web-and-plan.md)); the current roster is `apps/cli/tests/web-agent-presets.e2e.ts`'s own exact-roster assertion, not this paragraph.
