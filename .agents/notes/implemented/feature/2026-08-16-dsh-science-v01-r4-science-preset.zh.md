...
在 R4 阶段，精确模型工具名单为 `ask_user_question`、`get_science_state`、`glob`、`grep`、`read`、`read_image`、`run_python`、`run_r`、`skill` 与 `todo_write`，排除 bash/pwsh、文件系统写入/编辑、`str_replace_editor`、jobs、Goal、plan mode、subagents、workflows、Ralph、Code Mode、self-modification、Web search 与 chart/Outcome 发布。`annotate_artifact`、`install_science_packages`、`exit_plan_mode`、`web_search` 与 `web_fetch` 是之后加入的（[R5 charts/Outcome](2026-08-16-dsh-science-v01-r5-charts-outcome.zh.md)、[装包](2026-09-01-science-package-install.zh.md)、[web 与 plan](2026-09-01-science-web-and-plan.zh.md)）；当前名单以 `apps/cli/tests/web-agent-presets.e2e.ts` 自身的精确名单断言为准，而非本段。
