...
| Shipped composition | [R4](2026-08-16-dsh-science-v01-r4-science-preset.md) literal `science` preset and Web/CLI snapshot | The two tools plus Client/Host rows; the Runtime row stays deployment-owned |
...
The shipped Web composition mounts `session-attachment-index`, the Science Session Host plugin, and `ui-science`; it adds no `science-runtime` row. R5 preserves R4's deployment ownership: a live-capable deployment must still mount `@deepseek-ai/dsh-science-runtime` separately with a `science` profile, while the default Web application keeps the existing first-use missing-service/profile diagnostic. CLI/headless compositions without a browser keep the two model-facing tools and text fallback. The exact Science roster is the R4 roster plus `save_chart` and `publish_outcome`. Standard and other presets acquire neither tool nor the Science UI behavior through a process-global registration.
...
**Mount a default Science Runtime row in the base or Web bundle.** Rejected because R2 requires an explicit non-empty allowlist of existing Host Conda prefixes, and R5 neither discovers nor manages those paths. The shipped Web bundle gains the projection, attachment-index, and UI consumers; live creation continues to require a deployment overlay, and replay acceptance uses a stored fixture rather than implying a default Runtime.
...
The Science preset exposes exactly five Science tools — `get_science_state`, `run_python`, `run_r`, `save_chart`, and `publish_outcome` — and no other preset exposes either new tool. The shipped base and Web bundles add no Runtime row and keep R4's loud missing-service/profile behavior, so live-capable acceptance mounts an explicit deployment or test overlay. A deployment without an attachment store now leaves the Science Runtime waiting for `attachments`, which is the same fail-loud posture as a missing `science` profile and is visible in Loader diagnostics.
