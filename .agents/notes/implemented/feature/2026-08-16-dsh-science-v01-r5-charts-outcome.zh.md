...
|---|---|---|
| 已验收产品基线 | [R4 closure](2026-08-16-dsh-science-v01-r4-science-preset.md) head `fb04b0d273a6d4d3a319a4e8243c44953010f930` | 准确实现基线 |
| R5 inventory row | [`SCI-CHARTS-OUTCOME`](../../../../docs/evidence/2026-08-15-dsh-science-v01-r0-rc5-baseline-closure.md) | 唯一产品 delta |
...
| 已交付 composition | [R4](2026-08-16-dsh-science-v01-r4-science-preset.md) literal `science` preset 与 Web/CLI snapshot | 加入两个工具和 Client/Host rows；Runtime row 仍由部署拥有 |
...
已交付 Web composition 挂载 `session-attachment-index`、Science Session Host plugin 与 `ui-science`；不增加 `science-runtime` row。R5 保留 R4 的部署归属：具备实时能力的部署仍须另行挂载带 `science` profile 的 `@deepseek-ai/dsh-science-runtime`，默认 Web 应用则保留现有首次使用时的 missing-service/profile diagnostic。没有 browser 的 CLI/headless composition 保留两个模型工具与文字 fallback。准确 Science roster 是 R4 roster 加 `save_chart`、`publish_outcome`。Standard 与其他 preset 不会通过 process-global registration 获得任一工具或 Science UI behavior。
...
R5 不 supersede 任何 active Agent Note。R1-R4 保持 active，因为它们的持久语义、Runtime 归属、Consumer rules 与已交付 preset constraints 继续约束 Science line。[durable attachment](2026-07-22-web-multimodal-image-input-and-durable-attachments.md)、[minimal `read_image`](2026-08-10-minimal-read-image-tool.md)、[tool presentation](../architecture/2026-08-08-client-tool-presentation-ownership.md)、[Session projection](../../proposed/architecture/2026-07-27-session-projection-and-command-log.md) 与 [Session export](2026-08-10-web-session-log-export.md) 决策也继续独立有用；R5 消费它们，但不替代其 rationale。proposed [Task Surface](../../proposed/feature/2026-08-04-task-surface.md) 明确排除 charts，与本决策无关。
...
**在 base 或 Web bundle 挂载默认 Science Runtime row。** 拒绝，因为 R2 要求显式且非空的既有 Host Conda prefixes allowlist，而 R5 既不发现也不管理这些路径。已交付 Web bundle 增加 projection、attachment-index 与 UI consumers；实时创建继续要求 deployment overlay，replay acceptance 使用 stored fixture，不暗示存在默认 Runtime。
...
Science preset 准确暴露五个 Science tools——`get_science_state`、`run_python`、`run_r`、`save_chart`、`publish_outcome`——其他 preset 都不暴露两个新工具。已交付 base 与 Web bundles 不增加 Runtime row，保留 R4 的显式 missing-service/profile behavior，因此具备实时能力的验收会挂载明确的 deployment 或 test overlay。没有 attachment store 的部署现在会让 Science Runtime 停在等待 `attachments` 的状态，这与缺少 `science` profile 是同一种显式失败姿态，并可在 Loader diagnostics 中看到。
