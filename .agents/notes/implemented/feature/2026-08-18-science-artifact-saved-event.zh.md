...
本笔记最初推迟的 `attachment: ImageAttachmentRef | TextAttachmentRef` union，连同 `applyArtifactSaved` 原本仅限成功来源 run 的校验、以及每次保存都消费一次全新 tool call 的做法，一并在 runtime 自动捕获中落地；参见 [Science Runtime auto-capture of run-written files](2026-08-19-science-auto-capture.zh.md)。
