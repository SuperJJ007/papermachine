...
本笔记的 Decision 一节取代了[Science artifacts — versioned figures carrying their provenance](2026-08-18-science-artifacts.md)（同一功能线中的一篇已实现笔记）中"The artifact panel — Details column"与"The provenance view — conversation view tab"两节——该笔记这两节的正文现已直接陈述本次重设计的结果（版本导览条、header-actions 溯源/放大控件、按会话门控的 `conversation.view` 标签页——这两节最初提议、后被本次重设计取代的设计）。该笔记中 Host 端的投影关联与包清单相关决策不受影响，保持现行有效。
