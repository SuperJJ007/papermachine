...
**Two more additive owner-share fields.** Neither existed when the two seams above were opened; both surfaced only once the artifact panel's header controls and the provenance tab needed a write path the note's original two seams did not cover:
...
- `packages/client/ui-conversation/src/client/` — `contract/slots.ts` (the `conversation.details.header.actions` keyed slot and `ChatNodeOwnerProps`/`ChatViewInjected.openDetailsView`; the additive fields `DetailsHeaderActionOwnerProps.openView` and `ConvViewOwnerProps.inspectCall`), `skeleton/DetailsPanel.tsx`, `skeleton/DetailsPanel.module.css`, `skeleton/ConversationSession.tsx` (the `inspectCall` closure moves here from `ChatViewInjected`), `apply.ts`, `chat/ChatView.tsx`, `chat/ChatNodeSeat.tsx`.
