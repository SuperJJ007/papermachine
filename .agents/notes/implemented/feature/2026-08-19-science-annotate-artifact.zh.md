...
**在这里就把 `ScienceChartPresentation` 泛化为 `ScienceArtifactPresentation`。** 已拒绝：`ui-science` 是本决策不涉及的独立包；[per-media-type artifact viewer rendering (csv/json/md/png)](2026-08-19-science-viewer-file-types.md) 拥有 presentation 改名与分类型 dispatch。对非图片策展返回 `null` 是一个正确的、可用的过渡状态——而不是一个半成品类型。
...
**推迟到 [per-media-type artifact viewer rendering (csv/json/md/png)](2026-08-19-science-viewer-file-types.md)**：`ui-science` 中 `ScienceDetailsView.tsx`/`ScienceOutcomeRow.tsx` 的 `hasImageAttachment` 过滤器、`ScienceChartRow.tsx` 的 toolview key(仍是 `save_chart`，现在已不可达，因为不再存在同名工具——死代码但仍可编译，对用户不可见，因为不会再发生任何 `save_chart` 工具调用)，以及 `ScienceChartPresentation` → `ScienceArtifactPresentation` 的改名，都保持 [Science Runtime auto-capture of run-written files](2026-08-19-science-auto-capture.md) 落地时的原样，未作改动。
