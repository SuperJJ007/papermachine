...
**推迟到、并由 [per-media-type artifact viewer rendering (csv/json/md/png)](2026-08-19-science-viewer-file-types.md) 收尾**：`ui-science` 中 `ScienceDetailsView.tsx`/`ScienceOutcomeRow.tsx` 的 `hasImageAttachment` 过滤器、`ScienceChartRow.tsx` 的 toolview key(`save_chart`，因不再存在同名工具而不可达——死代码但仍可编译，对用户不可见，因为不会发生任何 `save_chart` 工具调用)，以及 `ScienceChartPresentation` → `ScienceArtifactPresentation` 的改名，都一直保持 [Science Runtime auto-capture of run-written files](2026-08-19-science-auto-capture.md) 落地时的原样，直到那篇笔记把 viewer 泛化到每种被捕获的媒体类型，并把会话记录行重新以 `run_python`/`run_r`/`annotate_artifact` 为键注册为止。
