...
`run_python` and `run_r` accept the same optional `artifact_inputs` and `edit_of` arrays. Each item uses the model-visible `{artifactId, version, path}` fields. An `artifact_inputs` path is relative to the run's reserved `inputs/` directory; an `edit_of` path is relative to `SCIENCE_ARTIFACT_DIR` and names the exact parent version for that output. The Consumer brands opaque ids and converts the arrays to `StartScienceRunRequest.artifactInputs` and `.editBaselines`; the Runtime remains the only owner of version resolution, path safety, byte/count bounds, materialization, and capture attribution.
