...
The browser selection store now holds an ordered union of artifact and file tabs. Artifact tab identity remains `artifact:<artifactId>` and continues to deduplicate a logical artifact while changing its selected version in place. File identity is `file:<relative path>`. A null active tab displays the library home without closing other tabs. The Details header selects its Artifacts or Project files page, while one shared document-tab strip appears below only when documents are open. Closing the last document returns to the selected library page.
