...
浏览器 selection store 现在保存 artifact tab 与 file tab 组成的有序联合。artifact tab id 仍为 `artifact:<artifactId>`，同一逻辑 artifact 继续去重，并在原 tab 中切换所选版本。file tab id 为 `file:<relative path>`。活动 tab 为 null 时选中固定的文件库主页 tab，但不关闭其他 tab；关闭最后一个文档后回到主页。
