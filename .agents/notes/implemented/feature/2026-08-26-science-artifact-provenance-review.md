...
Every artifact version now projects its producer Session id from the project artifact store. When that id differs from the current Session, a version opened from the project file library renders the source Session title and a disabled “Back to original conversation” action with an unavailable-navigation hint. It exposes no trajectory action because the current viewer binding cannot inspect another Session. Cross-Session navigation remains deferred until the browser owns a Session-switch route with a source anchor.
...
Chat owns semantic-anchor scrolling and saved reader position, while the conversation service owns the one-shot handoff across view mounting. Provenance contains a bounded causal summary with navigation, not copied dialogue. Notes persist within one Session only; the store-backed file library does not project them into a cross-Session read-only tab. Artifact events and their client projection retain `producerSessionId`, so a browser can distinguish local and foreign provenance without inferring from run ids.
