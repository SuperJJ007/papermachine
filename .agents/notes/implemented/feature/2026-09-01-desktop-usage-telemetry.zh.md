...
`apps/telemetry-receivers` 包含两个部署目标和一个共享的严格事件校验器。两端都只接受 `POST`，body 超过 8 KiB 后就不再缓存，在持久化前拒绝格式错误或非规范记录，返回空响应，不提供鉴权或 CORS 行为，并且绝不读取或保存 IP 地址、User-Agent 值或请求元数据；唯一例外是 `Content-Length`。由于阿里云目标会把完整的已接受对象写入 SLS，未知 body 字段也属于非法输入；这样可防止客户端意外增加字段后扩大 telemetry 存储的内容。阿里云 Node.js Web 函数监听 `9000` 端口，每个已接受请求写一行裸 `JSON.stringify(event)` stdout；它不保存实例内去重状态，SLS 查询按 `eventId` 去重。Cloudflare Worker 把 D1 绑定为 `DB`，填写 `received_at`，并在返回 `204` 前等待针对 `event_id` 主键的 `INSERT OR IGNORE` 完成，因此重复事件是成功的空操作。workspace 固定使用 Wrangler `4.127.1`，保证 Worker 与 D1 部署可复现，并明确允许 workerd 已审查的预编译平台选择安装脚本。`apps/telemetry-receivers` 是一个只用于部署的 package（Worker/Function 代码直接推送到云厂商，从不被任何消费者安装），因此 `scripts/check-workspace-constraints.ts` 用一个显式的 `deploymentOnlyAppDirectories` allowlist 命名它：它必须保持 `private: true` 且不携带 `publishConfig`，并且豁免其他 release-member apps 都要满足的 `apps/*` 发布文件策略。
...
`apps/telemetry-receivers/tests` 让共享校验器覆盖全部事件 variant 和非法 envelope——包括接受 `win32` 与 `darwin` 并列、以及拒绝未列入的平台（例如 `linux`）——覆盖阿里云 HTTP server 的空 `204`/`400`/`405`/`413` 响应与 stdout payload，并覆盖 Cloudflare Worker 的 D1 写入、重复成功、非法请求不写入，以及空 `500` 行为。
