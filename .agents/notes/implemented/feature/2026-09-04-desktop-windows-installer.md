...
`package:win` needs a shell that accepts `&&` chains and a leading environment assignment — Git Bash on Windows, not PowerShell. The workflow sets `shell: bash` for that reason.
