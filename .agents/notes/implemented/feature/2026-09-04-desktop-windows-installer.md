...
`apps/desktop/scripts/**/*.ts` joins `scripts/**/*.ts` in the repo-wide host face (`tsconfig.host.json`), which type-checked none of them before. That face rather than the package's own aggregate, because `stage-host.ts` imports the repository's `scripts/pnpm-invocation.ts` and the package project's `rootDir` cannot reach outside `apps/desktop`. The coverage immediately caught a narrowing bug in the first version of `write-update-metadata.ts`'s argument handling.
