...
- **Routes are concise and temporary material retires.** Each governed subtree uses a short `AGENTS.md` to route architecture, interface, documentation, workflow, and decision facts to their owners; `CLAUDE.md` is a symlink to the same instructions. Plans, handoffs, scratch notes, and generated investigation fragments live only under ignored `tmp/agent-work/<task>/`, carry an owner, creation date, and retirement condition, receive no inbound links from durable documents, and are deleted after durable facts move to their owner.
- **Dated evidence is a separate tier.** `docs/evidence/` stores exact-identity version and acceptance snapshots named by investigation date. Each record separates observed facts, inferences, recommendations, and unverified items and limits every PASS claim to its source, artifact, runtime, platform, and command scope. Evidence snapshots never replace current architecture or imply that a later revision inherited an earlier result.
...
- Route files remain cheap to load, architecture and interface references have explicit owners, and a finished task cannot leave temporary prose that competes with current documentation.
- Version and acceptance investigations remain auditable without copying moving SHAs, package versions, or PASS claims into current architecture.
