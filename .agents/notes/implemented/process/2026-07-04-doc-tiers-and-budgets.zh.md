...
- **路由文件保持精简，临时材料及时退役。**每个受治理的子树使用简短的 `AGENTS.md`，把架构、接口、文档、工作流和决策事实路由到各自归属处；`CLAUDE.md` 是指向同一说明的符号链接。计划、handoff（交接记录）、草稿和自动生成的调查片段只能放在被忽略的 `tmp/agent-work/<task>/` 下，必须注明负责人、创建日期和退役条件，不得被持久文档链接，并在持久事实迁入归属文档后删除。
- **日期化证据是独立层级。**`docs/evidence/` 保存按调查日期命名、带确切身份的版本与验收快照。每份记录分开观测事实、推断、建议与未验证事项，并把每项 PASS claim 限定到其 source、artifact、runtime、platform 与 command 范围。证据快照绝不替代当前架构，也不暗示后续 revision 继承早期结果。
...
- 路由文件保持低加载成本，架构与接口参考拥有明确归属，已完成任务不会留下与当前文档竞争的临时行文。
- 版本与验收调查保持可审计，同时避免把移动中的 SHA、package version 或 PASS claim 复制进当前架构。
