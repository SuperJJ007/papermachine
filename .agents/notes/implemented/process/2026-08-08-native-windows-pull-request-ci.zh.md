...
Science 生命周期测试保持协议和所有权断言可跨平台执行，即使原夹具依赖 POSIX shell、FIFO 或 chmod 故障。测试专用适配器和可控故障观察让这些断言也在 Windows 上执行；真实平台集成仍单独验证。夹具限制不能成为跳过已支持运行时行为的理由，模拟元数据也不能替代原生安全验收。
