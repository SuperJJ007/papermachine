...
The partition blob reporter copies coverage positions and encodes an infinite source column as `Number.MAX_SAFE_INTEGER` before JSON serialization. V8 source mapping uses infinity for unmapped line ends; serializing it as `null` prevents Istanbul from recognizing containing ranges when Node and jsdom transforms meet across partitions. The finite bound preserves containment and leaves every hit count unchanged, including zero counts. The provider map remains untouched for in-process reports.
...
Reporter tests preserve finite and unbounded positions, covered and zero counters, and the original provider map. A two-partition home-paths run with Node and jsdom imports reaches the same per-file 100% result as an unpartitioned run; omitting the default-home case still fails the merged branch threshold.
