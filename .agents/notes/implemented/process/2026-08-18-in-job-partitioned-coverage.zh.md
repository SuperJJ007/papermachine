...
Linux 与 Windows 使用相同的协调器，并各自设置分区数量与外围 worker 预算。两条 PR 通道都通过 `DSH_COVERAGE_TEST_TIMEOUT_MS` 为测试和钩子分配 90 秒；冷启动编译器分析和原生进程清理与其他插桩分区共享运行器，本地默认的 5 秒无法约束它们在负载下的执行时间。两条拉取请求覆盖率通道都以零构建方式运行；完整参考流程（serial-windows standby）仍在构建门禁之后启动其覆盖率门禁，使消费构建产物的套件对真实产物执行。本地覆盖率默认保持简单；只有调用方显式选择分区包脚本并提供大于 1 的合法数量时，才启用分区。
