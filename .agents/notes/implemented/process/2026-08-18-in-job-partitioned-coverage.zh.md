...
分区 blob reporter 会复制覆盖率位置，并在 JSON 序列化前把无限 source column 编码为 `Number.MAX_SAFE_INTEGER`。V8 source mapping 用 infinity 表示未映射的行尾；若把它序列化为 `null`，Istanbul 就无法在跨分区合并 Node 与 jsdom 转换时识别包含关系。有限上界保留包含语义，所有命中次数（包括零次）均保持不变。进程内报告使用的 provider map 不会被修改。
...
Reporter 测试固定有限与无界位置、已覆盖与零命中计数，以及原始 provider map 的不变性。包含 Node 与 jsdom import 的 home-paths 双分区运行达到与非分区运行相同的逐文件 100%；省略默认 home 测试后，合并分支阈值仍会失败。
...
Linux 与 Windows 使用相同的协调器，并各自设置分区数量与外围 worker 预算。两条 PR 通道都通过 `DSH_COVERAGE_TEST_TIMEOUT_MS` 为测试、断言轮询和钩子分配 90 秒。每个内联 Vitest 项目直接接收该配置：Vitest 4 会向项目传递命令行测试超时，却不会传递命令行钩子与轮询超时。协调器仍为根调用保留这些命令行参数；冷启动编译器分析和原生进程清理与其他插桩分区共享运行器，本地默认的 5 秒无法约束它们在负载下的执行时间。两条拉取请求覆盖率通道都以零构建方式运行；完整参考流程（serial-windows standby）仍在构建门禁之后启动其覆盖率门禁，使消费构建产物的套件对真实产物执行。本地覆盖率默认保持简单；只有调用方显式选择分区包脚本并提供大于 1 的合法数量时，才启用分区。
