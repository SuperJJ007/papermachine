...
`scripts/rescope-vendor.ts` 里的 `GENERIC_SKIPS` 为受影响的每个文件各增加一条记录，都把 `cordis` 记成被跳过的上游 token，沿用这个文件自己既有的裸词碰撞处理方式（与 `ui-agent-preset` 那一组已经在用的机制相同），而不是新增一条匹配规则。[`docs/rescope.md`](../../../../docs/rescope.md) 及其中文版在"改名不碰什么"下新增一条，与既有的 Loader `cordis:` 前缀、`cordis.yml` 配置文件家族两条并列。
...
- [改名映射文档](../../../../docs/rescope.md)与本篇 Note，给下一个撞见这个功能里以 `cordis` 开头的标识符的人留了一个有据可查的先例，不用重新诊断一遍；[最初的改名 Agent Note](2026-08-10-vendor-package-rescope.md) 记录着这次豁免所依附的那个映射决策。
