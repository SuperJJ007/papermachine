...
A fresh Harness home opens onboarding before the session workspace. The user selects a discipline package, enters or skips a DeepSeek API key, reviews the required download size, and starts provisioning. The key is written through the existing writable credentials provider as `DEEPSEEK_API_KEY`; it never enters desktop settings, logs, renderer persistence, command arguments, or crash diagnostics. Skipping the key keeps the application usable and presents the existing model-configuration guidance when a session needs inference.

Onboarding state records only non-secret progress and the selected discipline under the Harness home. Completion requires a verified environment revision, not merely a dismissed window. A failed step presents the failing subject, a safe correction, retry, and access to diagnostics with secrets redacted. Reinstalling or upgrading the application reuses the Harness home and does not repeat provisioning when the declared revision and health checks still match.
