...
onboarding 现在会经过本节，作为其第二条路径——安装——与检测并绑定机器上已有的 conda-family 环境并列（见下文「首启向导与凭据」）：见 [general 环境安装路径 Agent Note](../../implemented/feature/2026-09-01-desktop-general-environment.zh.md)，它同时用一个 `general` 声明取代了下文的 social-science/biology 声明，并新增了用户自行编写的 `custom` 声明。本节所述的 declaration schema、事务性 prefix 安装、health checks 与可恢复性保持如上所述。

每个 shipped environment declaration 命名支持的平台、micromamba environment lock 或明确的 channels 与 packages、磁盘空间提示以及 health checks。声明不含 executable hooks。桌面 provisioning service 校验声明，使用应用内嵌的 micromamba executable 安装到应用持有的 environment directory，把结构化进度流送到 onboarding UI，并且只在两个 interpreter health checks 都通过后发布 applied Science environment revision。
...
每个 slice 都从限定该 slice 范围的 implementation brief 开始，只有通过 package tests、在 model 或 product output 改变时提供 assembled keyless snapshot，并同步 docs 与 Agent Note 之后才算落地。onboarding 之后被重做：先从「学科选择 + micromamba provisioning」改为「检测并绑定已有的 conda-family 环境」，随后 [general 环境安装路径 Agent Note](../../implemented/feature/2026-09-01-desktop-general-environment.zh.md) 又把 micromamba 路径重新接回 onboarding，作为与检测并绑定并列的安装选项。还有四项属于 outstanding attended evidence——需要有人在真实硬件上运行并记录的验证，而非尚未实现的 slice：一次在全新 macOS 账户上、针对已有 Anaconda（或等价物）安装的真实检测并绑定首次运行、一次在全新 macOS 账户上的真实 DMG 安装与首启 onboarding、一次验证 preload 能在 built、已签名应用的 `sandbox: true` 下加载的 packaged-preload smoke test，以及确认 packaged watchdog 在 Electron 内嵌 Node 下 `import.meta.main` 确实为真——没有自动化测试覆盖这条生产环境的入口判断，若其值为 undefined，watchdog 会悄无声息地什么也不做。Downloads 与 real-provider checks 始终作为显式 evidence rows，绝不从 source tests 推断。
...
**只用内嵌 micromamba 配备 Python 与 R 作为 onboarding 唯一路径，不提供检测并绑定选项。** 否决：一次全新的、从数百 MB 到数 GB 不等的下载与安装，会与目标科研者大多已经通过 Anaconda 或实验室统一管理的 Conda 安装重复。为此优先提供检测并绑定，micromamba provisioning 作为面向没有可检测环境的机器的第二条路径（见上文「环境配备」）。绑定用户自行管理的环境，会让环境可复现性依赖 host machine 自身的 Conda 安装，而不是一份 pinned declaration——这是检测并绑定路径的既定取舍，而非放弃它的理由。
...
- 在已经装有 conda-family 环境（Anaconda、Miniconda 或等价物）的受支持 Mac 上，用户安装 DMG、通过检测并绑定该环境完成 onboarding、分析本地 dataset、创建 Python 与 R charts 并导出 artifact，全程无需终端或 browser URL。在完全没有 conda-family 安装的受支持 Mac 上，onboarding 的安装路径（见上文「环境配备」）会下载并配备 shipped 的 `general` 环境，无需终端也无需安装 Anaconda。
