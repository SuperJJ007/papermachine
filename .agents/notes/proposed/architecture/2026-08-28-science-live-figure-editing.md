...
Every surface that names an artifact outside a version-scoped detail — the library card, the viewer header, the tab label, the composer chip, and the per-turn artifact card — shows the artifact's latest known version's curated title, fixed as a user steps between versions; only the version badge and origin marker change with the exact open version. The provenance drill-in is exempt: it names the exact version it describes.

A model that self-checks a render with `read_image` and re-renders a fix in the same turn, before curating a title, produces two genuinely different-byte versions in one turn; the earlier one reads to a user as a stray near-duplicate of the one right after it. The fix is display, not the version model: a version is a same-turn intermediate draft when a strictly later version of the same artifact shares its authorizing turn — the client artifact projection's `turn` field, resolved server-side from the folded `tool/call` facts rather than reconstructed client-side from raw session events — and it is not itself a human-edit save. The version stepper walks only non-collapsed versions by default, behind an expandable entry that temporarily includes the collapsed ones; the provenance drill-in and the library/turn cards, which already show only the latest version, are unaffected.
