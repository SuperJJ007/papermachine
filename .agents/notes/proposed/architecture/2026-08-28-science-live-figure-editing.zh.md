...
封闭操作集合为 `set_title`、`set_axis_label`、`set_legend_position`、`toggle_grid` 与 `set_font`。两个适配器都为全部五项定义确定性行为，只有标题／副标题、x/y 轴标签、图例、网格与字体元素暴露直接控件。其余每个已抽取元素都保持可见，并可作为精确引用发送给模型。字体元素只公开当前字体族与字号，绝不枚举已安装字体族。字体操作接受有界的自由文本字体族与字号；runtime 无法精确解析字体族时以 `font_not_found` 失败，且不改变图对象，也绝不修改 matplotlib 的全局 `rcParams`。紧凑表单把直接修改行与引用芯片排成并排两栏；详情列窄于 440px 时两者上下堆叠，引用芯片在直接修改下方。
