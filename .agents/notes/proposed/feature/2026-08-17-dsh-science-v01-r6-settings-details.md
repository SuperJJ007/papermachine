...
**R6c-0 — path-addressed settings writes.** Start from the accepted R6c implementation base named in the identity table. Add a path-addressed `set`/`unset` to the browser-side `SettingsScope` contract and its `SettingsScopeController`, with focused tests for a nested write, a nested clear, revision fencing on both, and the queue/recovery behavior the single-segment methods already own. Touch no Science-owned file and no Host or wire code. Independently review and accept the exact R6c-0 head before R6c starts; propose the same change upstream separately, since nothing in it is Science-specific.

**R6c — Science product composition and closure.** Start from the accepted R6c-0 head. Add the Science settings card/header action/Details entry, the default Web Runtime row, package metadata and invariants, assembled keyless browser coverage, accessibility checks, a GIF recorded from the real server and model flow as the GUI-change rule requires, packed Web evidence, real Python/R Runtime acceptance, current documentation, the implemented rewrite of this Note, and a dated R6 evidence triplet. Final review covers the accepted R6c base through the R6c head and stops on any source, privacy, browser, packed, or real-runtime failure.
...
- R6a and R6b started only from the clean R5 closure head `16f5ce76abf8483c42bf02214cf15d82a2300b9c`, whose implemented Note, dated evidence, and browser-safe projection agree; its one later correction, `58aee8561ca665fc7056f2dc013e7012aafc4da5`, regenerates the module graph R5 left stale and records the miss in the R5 evidence. R6c-0 and R6c start only from the accepted post-rebaseline head the identity table names, on a clean worktree.
- The path-addressed `set`/`unset` lands in `ui-settings` as its own accepted head before any Science-owned file changes, and the Science card writes `profiles.science.pythonPrefix`/`rPrefix` through it. No code path writes the whole `profiles` field from the browser.
...
- Final independent review receives the accepted R6c base, the R6c head, this scope, command evidence, privacy sentinels, and protected-worktree snapshot, reproduces the critical checks, and reports no unresolved high-severity finding before the Note moves to implemented.
