...
The desktop application defaults to `partial` on Windows and `full` on macOS for Science probes, kernels, and package installs, without a confirmation dialog. `PAPERMACHINE_SCIENCE_MINIMUM_ENFORCEMENT` overrides this policy with `full` or `partial`. Invalid values reject Host preparation. Set it on the Electron process at launch; both staged and active profiles receive the same policy. The Windows ACL backend reports `partial`, with the [documented write-restriction limits](../../packages/sandbox/sandbox-windows-acl/README.md); accepting it does not change the backend or provide full isolation.
...
| `store-archives.json` and `store-archives/*.tar` | Validate the deterministic uncompressed shards, asynchronously extract them into a unique Desktop staging directory and copy matching immutable store files, and transactionally merge pnpm's versioned SQLite package index into `$DSH_HOME/desktop/pnpm/store` without removing packages already downloaded for Desktop plugins. |
...
For local macOS installed-app acceptance, run `DSH_DESKTOP_LOCAL_ACCEPTANCE=1 pnpm run package:desktop:mac:arm64` (or the `mac:x64` command on a compatible host). This mode ad-hoc signs the application and seed executables, verifies the resulting signatures, and retains the offline seed installation and Host health checks. It requires no Developer ID, notarization credentials, or update origin. Its resources and `-local-` installers live under `.desktop-build/local-acceptance/<target>/`; it emits neither update configuration nor a release completion record, and the upload command rejects this mode. Production update selection and unsupported platforms are rejected. Local acceptance qualifies installed behavior, not Gatekeeper distribution or signed updates; release packaging retains the requirements below.

On Windows, set `DSH_DESKTOP_LOCAL_ACCEPTANCE=1` and `DSH_DESKTOP_LOCAL_WINDOWS_CERT_THUMBPRINT` to the 40-character thumbprint of an existing Code Signing certificate with a private key in `LocalMachine\\My`, then run `pnpm run package:desktop:win:x64`. Local signing uses SHA-256 Authenticode for the application, installer, and temporary NSIS bootstrap, retaining the generated PE certificate-table repair. A self-signed test certificate is sufficient; packaging neither changes certificate trust nor exports private keys, and accepts a valid signature whose certificate is untrusted. Windows execution policy and application-control decisions still apply. Resources, installers, and upload refusal use the same local isolation as macOS; no SafeNet release credentials are required.

Release packaging is one complete command. It performs release preparation before creating the host platform's installers and update metadata. Every target defaults to the existing PaperMachine application ID `com.papermachine.desktop`; an explicit reverse-DNS `PAPERMACHINE_DESKTOP_APP_ID` overrides it. Keep the same ID across upgrades. The application is named `PaperMachine`, and installer names start with `papermachine-`. macOS targets additionally require the electron-builder certificate qualifier in `DSH_DESKTOP_MACOS_SIGNING_IDENTITY`, its 10-character Apple Team ID in `DSH_DESKTOP_MACOS_TEAM_ID`, and one complete notarytool credential strategy. The App Store Connect API-key strategy uses these variables:
