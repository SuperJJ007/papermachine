...
暂存的 Host closure 带着每个桌面打包 target 的 `sharp`/`koffi` 原生模块变体——做到这一点靠的是 `pnpm-workspace.yaml` 的 `supportedArchitectures`：没有它，`pnpm install` 只会拉运行机器自己那一份 darwin-arm64、darwin-x64 或 win32-x64 变体。`scripts/stage-host.ts` 在暂存完成后立即断言三者都在，缺任何一个就报错失败并指明修复方式；`scripts/after-pack.mjs` 随后删掉所有不属于这次 electron-builder 调用自身平台与架构的变体（`scripts/native-module-targets.mjs` 持有两步共用的纯选择逻辑）。没有这一步，在 arm64 mac 上打的 x64 DMG、或在 mac 上交叉打的 Windows 包，都会把宿主机自己的 darwin-arm64 二进制带进另一个平台的 app——sharp 与 koffi 都在 Host 启动时加载，打包出来的 Host 会在就绪前退出且构建期毫无信号。见[按目标平台携带原生模块的 Agent Note](../../.agents/notes/implemented/bug-fix/2026-09-07-host-native-modules-per-target.zh.md)。
