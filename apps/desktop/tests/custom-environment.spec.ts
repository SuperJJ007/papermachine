 ...
 * shipped parser, a revision that tracks the set's content but not its
 * sources, and the round-trip that keeps a custom install resolvable on the
 * next launch.
 */
...
const OFFICIAL: readonly EnvironmentSource[] = [
  { id: 'official', name: 'Official channel', channels: ['https://conda.anaconda.org/conda-forge'] },
]
const MIRRORED: readonly EnvironmentSource[] = [
...
describe('buildCustomDeclaration', () => {
  it('publishes the set under the custom id with the requested packages and sources', () => {
    const declaration = buildCustomDeclaration(['python=3.13', 'scipy'], ['darwin-arm64'], OFFICIAL)

    expect(declaration.id).toBe(CUSTOM_ENVIRONMENT_ID)
    expect(declaration.packages).toEqual(['python=3.13', 'scipy'])
    expect(declaration.sources).toEqual(OFFICIAL)
  ...
  it('gives the same package set the same revision regardless of the order it was listed in', () => {
    const one = buildCustomDeclaration(['scipy', 'python=3.13'], ['darwin-arm64'], OFFICIAL)
    const other = buildCustomDeclaration(['python=3.13', 'scipy'], ['darwin-arm64'], OFFICIAL)
  ...
  it('gives a changed package set a different revision, so it provisions into its own prefix', () => {
    const before = buildCustomDeclaration(['python=3.13'], ['darwin-arm64'], OFFICIAL)
    const after = buildCustomDeclaration(['python=3.13', 'scipy'], ['darwin-arm64'], OFFICIAL)

    expect(before.revision).not.toBe(after.revision)
  })

  it('keeps the same revision when only the sources change, since sources describe where packages come from, not which packages', () => {
    const withOfficial = buildCustomDeclaration(['python=3.13', 'scipy'], ['darwin-arm64'], OFFICIAL)
    const withMirror = buildCustomDeclaration(['python=3.13', 'scipy'], ['darwin-arm64'], MIRRORED)

    expect(withOfficial.revision).toBe(withMirror.revision)
  })

  it('rejects a token that would reach the solver argv as something other than a package spec', () => {
    expect(() => buildCustomDeclaration(['--prefix /tmp/evil'], ['darwin-arm64'], OFFICIAL))
      .toThrow(/invalid package token/)
    expect(() => buildCustomDeclaration(['scipy; rm -rf /'], ['darwin-arm64'], OFFICIAL))
      .toThrow(/invalid package token/)
  })

  it('rejects an empty package set', () => {
    expect(() => buildCustomDeclaration([], ['darwin-arm64'], OFFICIAL))
...
describe('custom declaration persistence', () => {
  it('round-trips the declaration the next launch resolves the applied environment against', async () => {
    ...
    const declaration = buildCustomDeclaration(['python=3.13', 'r-base=4.5'], ['darwin-arm64'], OFFICIAL)
  ...
  it('fails loudly on a corrupt file rather than silently dropping a working environment', async () => {
    const directory = await root()
    const declaration = buildCustomDeclaration(['python=3.13'], ['darwin-arm64'], OFFICIAL)
    await writeCustomDeclaration(directory, declaration)
