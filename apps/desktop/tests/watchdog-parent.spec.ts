...
import { watchParent } from '../src/watchdog-parent.ts'

const children: ChildProcess[] = []
const directories: string[] = []
function ownedSpawn(command: string, args: readonly string[], options: SpawnOptions = {}): ChildProcess {
...
describe.runIf(process.platform !== 'win32')('watchParent', () => {
  it('detects a real parent process disappearing via ppid reparenting, then stops the Host group', async () => {
    ...
    const host = ownedSpawn(process.execPath, ['--eval', 'setInterval(() => {}, 1000)'], {
      detached: true,
      stdio: 'ignore',
    })
    ...
    const parentSource = `
      ...
      setInterval(() => {}, 1000)
    `
    const parent = ownedSpawn(process.execPath, ['--eval', parentSource])
  ...
  it('escalates to SIGKILL when the Host group ignores SIGTERM', async () => {
    ...
    const host = ownedSpawn(process.execPath, ['--eval', "process.on('SIGTERM', () => {}); setInterval(() => {}, 1000)"], {
      detached: true,
      stdio: 'ignore',
    })
    if (host.pid === undefined) throw new Error('fixture process missing pid')
  ...
  it('stops a cooperative Host group with only SIGTERM', async () => {
    const alreadyGoneParentPid = process.ppid + 1
    const host = ownedSpawn(process.execPath, ['--eval', "process.on('SIGTERM', () => process.exit(0)); setInterval(() => {}, 1000)"], {
      detached: true,
      stdio: 'ignore',
    })
    if (host.pid === undefined) throw new Error('fixture process missing pid')
