...
it('accepts a whole-page drop under the limits-labeled overlay and refuses an over-limit batch at intake', async () => {
  ...
  const heading = await screen.findByText('Drag images here to add them')
  const overlay = heading.closest('[role="status"]')
  if (overlay === null) throw new Error('drop overlay has no live status region')
