...
describe('web e2e: resident question composer round trip', () => {
  ...
  it('asks through the composer, answers, and completes with the answer logged', async () => {
    ...
    await page.locator('[data-conversation-scroll]').evaluate((element) => { element.scrollTop = element.scrollHeight })
    await expect.poll(() => page.getByRole('button', { name: 'Back to bottom', exact: true }).count()).toBe(0)
