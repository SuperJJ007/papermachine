...
`agent/pre-step` decides what the model sees. Listeners may rewrite the claimed messages or reject them outright; a rejected or empty first claim still closes a durable turn that spent no step, so the log records the attempt. Each step reads the prompt sections and tool schemas that plugins registered. The final Enter batch is authoritative for a newly projected runtime-context candidate, while an already-retained current snapshot is restored if pressure compaction removes it during the waterfall. After the first-request value is fixed, a retry re-derives model history from the log and restores only that exact retained snapshot if a request-error handler removed it, without repeating prompt assembly ([agent-loop](../packages/core/agent-loop/README.md#loop-lifecycle-agentts)).
