...
The [Electron desktop application](../apps/desktop/README.md) owns the reserved `$DSH_HOME/profiles/desktop` npm project. Signed releases bind an exact dsh version and first-party offline seed. Bundled pnpm installs that version while preserving desktop-plugin versions. Science CLI and Desktop share PaperMachine data, isolated from official DSH, but keep executables, plugin activation, lockfiles and `node_modules` separate.
