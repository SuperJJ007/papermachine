...
| `ctx.scienceEdits` | `core` | [`tool-science`](../packages/science/tool-science) | - | `ui-science` | - | The Host provider validates exact live-Agent raster-region selections and queues structured model edit messages; the browser consumer never mutates the projection itself. |
