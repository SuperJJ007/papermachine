...
| `ctx.scienceEdits` | `core` | [`tool-science`](../packages/science/tool-science) | - | `ui-science` | - | Host provider 校验在线 Agent 的精确 raster-region 选择并排入结构化模型编辑消息；浏览器 consumer 本身绝不修改 projection。 |
