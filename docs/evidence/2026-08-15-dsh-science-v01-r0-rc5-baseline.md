...
The scope record's acceptance criteria name "installed CLI identity" as a required PASS layer bound to the literal minimum-verification-matrix command. That literal command (`pnpm run release:verify-packed-install --family dsh ...`) is FAIL on this host for the diagnosed, host-specific reason above, not because of any defect in `<R0B_HEAD>`'s content. The adapted verification directly above installs the identical packed tarball set and confirms the same underlying facts the literal command exists to prove (the packed dependency graph resolves and the installed CLI reports the expected version). This gap is recorded rather than silently closed; it is not resolved by editing `koffi`, the release workflow, or any dependency, which R0 does not authorize.
