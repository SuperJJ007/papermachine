...
The official [`deepseek-ai/deepseek-harness`](https://github.com/deepseek-ai/deepseek-harness) source has commit `47f943859bef60e4160492346772ded9b24f765a`, tree `f904efab9ef435201d6ba4da88a34d6366568272`, committed 2026-08-13T19:38:46+08:00 ("Merge pull request #2519 from deepseek-harness/feat/npm-public"). `package.json` at that commit reads `name: "@deepseek-ai/dsh-root"`, `version: "0.1.0-rc.5"`, `license: "MIT"`, `engines.node: "^22.19.0 || >=24.0.0"`, `packageManager: "pnpm@11.7.0"`. The commit object was already present in this repository's local object database before this investigation (no fetch was required; local and remote-tracking branch refs remained unchanged).
...
| R0B preliminary candidate | `codex/science-v01-rc5-baseline` | `/Users/superjj/ccproj/dshscience-r0b-rc5-baseline` | `948693150e2fe8a9d38fbb1e125a2a106d9488ee` (tree `35062cb234e05176fa83132253ea797634700a1a`) | `47f943859bef60e4160492346772ded9b24f765a` (official RC5, no downstream-history ancestor) ← `3751f6fcc497e9e23385f69a0d5f3cfdbaac5a6e` ← `922d60421a8a6a2983f27de266a02d3d7c5af3b3` ← `3ca64b38867fe78792a4d95ff3ba39e26444db82` ← `948693150e2fe8a9d38fbb1e125a2a106d9488ee` |
...
Every result below is bound to the R0B commit current at the time the command ran. The preliminary record closes at `948693150e2fe8a9d38fbb1e125a2a106d9488ee`; the later closure record owns final-head identity and protected-state acceptance.
...
| Identity and scope (preliminary) | `git rev-parse HEAD HEAD^{tree}`; `git merge-base --is-ancestor 47f943... HEAD`; `git diff --name-status 47f943...HEAD`; `git status --porcelain=v2 --branch` | Preliminary candidate `948693150e2fe8a9d38fbb1e125a2a106d9488ee`; the closure record owns the accepted-head resolution and final scope check |

### Acceptance-criteria gap

The scope record requires installed CLI identity through the literal minimum-verification-matrix command. That command (`pnpm run release:verify-packed-install --family dsh ...`) failed on this host for the diagnosed reason above; the R0B documentation content did not introduce that behavior. The adapted verification directly above installs the identical packed tarball set and confirms the packed dependency graph resolves and the installed CLI reports the expected version — but it deliberately does not pass `--omit=optional`, so it does not prove the literal official command's flag combination on another platform. The preliminary gap remains historical here; the closure record separately evaluates the literal command on the release workflow platform without editing `koffi`, the release workflow, or any dependency.

`koffi` is a general dependency (`packages/fs/fs-local`, `packages/host/directory-picker-native`, `packages/sandbox/sandbox-windows-acl`, `packages/session/session-persistence-jsonl`), not a Landlock-only one, so the preliminary record did not assume the issue was macOS-exclusive. Ubuntu behavior was unverified at this observation point; the closure record owns the later workflow-platform result.
...
| Built-in Science preset | `fae091e1080e830bed8ad0456e4cbced29101b01`, Science preset/config/snapshot paths | `Science-owned` | Candidate only; whole-candidate acceptance was FAIL; requires accepted tools and composition evidence |
...
At the time of the preliminary record, the GitHub source-archive SHA-256 had not been computed because the commit object was already present locally. The closure record later owns the archive download and checksum result. No RC6 source mapping or compatibility audit was performed. Real provider, real Python/R, Desktop, installer, signing, notarization, and publication readiness remain unknown and are not implied by any PASS above. A later revision does not inherit any result in this record without its own exact-identity rerun.
