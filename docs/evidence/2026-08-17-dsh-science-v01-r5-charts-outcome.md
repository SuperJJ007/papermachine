...
| Module graph | `pnpm run verify-module-graph` | **FAIL at the candidate, corrected after closure.** R5 added `client-ui-science` and `session-attachment-index` and gave `science-runtime` its `attachment` dependency without regenerating `docs/module-graph.md`. The roster above missed it because `doc-sync` did not carry this gate and the hygiene sub-check list does not include it; only `check-all` and the CI static lanes ran it. Regenerated and re-paired in the correction commit that follows this closure head |
