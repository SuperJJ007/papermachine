...
R6 closes `SCI-SETTINGS-SIDEBAR` on branch `codex/science-v01-rc7-rebaseline`. The accepted product candidate and final head is `ebd29f454a5b86445190f004cc5f51cbfb6c3f1a` (`fix(ui-science): judge settings-card save landing per field, not all-or-nothing`); `e125ce00327e4ffce9cc01f371b9068fd142dfcc` is an intermediate SHA in the chain, not the final head. R6a and R6b were already accepted on the pre-rebaseline tree; R6c's base is the [rc.7 rebaseline](2026-08-17-dsh-science-v01-rc7-rebaseline.md) head `66344a2774feaad7ebd27e80f11e6386d8255317` (itself descended from `24971d5f14c8b9dc692658a0bb1cab599a4ed526`, the accepted post-rebaseline head, through documentation-only R6c planning commits and R6c-0). Four commits carry R6c-0b through `e125ce0032` on the branch's actual ancestry: `6a994ef4cbca968e15b4ef3d63f0f8e1bb2613e1` (R6c-0b), `ef645eff0e3a1ab7c416202fe0b023a308ab6f8f` (settings card + default Web Runtime row + R4/R5 amendments), `57af4a3702956b15875769bf2f5c774e59d68a74` (header action + Details entry), and `e125ce00327e4ffce9cc01f371b9068fd142dfcc` (assembled browser/snapshot coverage + goldens, plus card chrome on shared primitives). `8558a65d77ab44f522039bf2624af93f241b7efa` is **not** an ancestor of the final head: `git branch --all --contains 8558a65d77` returns no branch, and the reflog records it as `commit (amend)` — `e125ce0032` amended `8558a65d77` in place rather than building on top of it, so both share the same parent `57af4a3702`. That SHA carries no unique evidence. Independent review of the checkpoint that closed at `e125ce0032` found one correctness defect in the settings-card controller's multi-field save bookkeeping (see Independent review follow-up below); the fix landed in `ebd29f454a`, a source change, so it is the binding final head throughout this record. Every attended and lane-level check below was rerun directly at `ebd29f454a` so no row points at a superseded SHA.
...
| Final head (product candidate) — independent-review fix, per-field settings-card save landing | `ebd29f454a5b86445190f004cc5f51cbfb6c3f1a` |
...
| Superseded pre-amend SHA (no unique evidence) | `8558a65d77ab44f522039bf2624af93f241b7efa` — not an ancestor of the final head (`git branch --all --contains` returns none; reflog: `commit (amend)`); fully superseded |
| R6c assembled browser/snapshot coverage + goldens + card chrome on shared primitives (intermediate SHA, superseded as final head by the row above) | `e125ce00327e4ffce9cc01f371b9068fd142dfcc` |
...
|---|---|---|
| Focused `ui-science` | Vitest, `packages/client/ui-science/tests` | PASS — 8 files / 106 tests, per-file coverage 100% on every changed src file (statements 295/295, branches 276/276, functions 81/81, lines 248/248) |
...
| Full Web lane, final head | `pnpm run test:web` (fresh `build` then `test:web:built`) | 2 files failed / 75 passed / 1 skipped (78 files); 6 tests failed / 261 passed / 7 skipped (274 tests) — 5 failures in `smoke-real.e2e.ts` (inherited), 1 in `background-job-list.e2e.ts` (known flake, recurred; confirmed non-reproducible standalone — `Test Files 1 passed (1)` / `Tests 3 passed (3)`) — see Disclosed inherited failures |
...
| Packed payload | `@deepseek-ai/dsh-client-ui-science` | PASS — 20 entries, including `lib/client.js` and declarations for all five new modules; no source leakage; unchanged in composition from the intermediate SHA |
| Real Python/R run 1 | `test:real-acceptance` at `ebd29f454a`, isolated home, both prefixes | PASS — `python.status=PASS`, `r.status=PASS`, no `prefixManifestDifferences`, `candidateSha=ebd29f454a5b86445190f004cc5f51cbfb6c3f1a` |
...
`.playwright-mcp/r6c-science-settings-card.gif` — 1200×750, 10.2 s, 173739 bytes, 4 source frames, 102 encoded frames. Re-recorded at the final head `ebd29f454a` (superseding the earlier 168413-byte recording taken at the intermediate SHA `e125ce0032`), specifically so the artifact binds the final head rather than an intermediate one. Same method as before: a real server booted off the built tree (`apps/cli/lib/bin.js --profile web --port 3099`) with a fresh `DSH_HOME`/`DSH_AGENTS_HOME`/workspace and the **default** Web composition (no overlay), driven by headless Chromium via the repository-declared Playwright 1.61.1, with zero page errors and the welcome notice dismissed by the script. It demonstrates the settings card end to end: collapsed among its siblings, expanded/unconfigured, both prefixes typed, saved with both fields reading "Configured", inputs empty on reload (no echo of a stored path), and restart-required. The Host settings document again received exactly the two-segment write this evidence's Verification matrix and the implemented Note both name (`science-runtime:\n  science:\n    pythonPrefix: …\n    rPrefix: …`).
...
| `subagent-interrupt-ui.e2e.ts` | One single-test flake, observed once during the pre-amend evidence pass | Non-reproducible then: passed standalone and in the other full run at that time. Did not recur in either full-lane rerun since — at the intermediate SHA `e125ce0032` or at the final head `ebd29f454a` |
| `background-job-list.e2e.ts` | One single-test flake, observed during the pre-amend evidence pass; absent when the lane reran at the intermediate SHA `e125ce0032`; recurred once more (1 of 274 tests) when the lane reran at the final head `ebd29f454a` | Non-reproducible each time it has appeared: passed standalone at the pre-amend pass, and passed standalone again at the final head (`Test Files 1 passed (1)` / `Tests 3 passed (3)`) — an intermittent, load-sensitive flake, not a regression |
...
Every disclosed failure above traces to one of three causes unrelated to this range: a pre-existing signature already confirmed set-identical to upstream rc.7 (`rescope-vendor:check`), a file-identity or interaction-mechanism argument tracing the exact cause to the rc.5 line with zero R6-range edits reaching it (`goal.snapshot.ts`, `smoke-real.e2e.ts`), or an intermittent single-test flake that has passed standalone every time it has been isolated (`subagent-interrupt-ui.e2e.ts`, observed once and absent from both full-lane reruns since; `background-job-list.e2e.ts`, observed at the pre-amend pass, absent at the `e125ce0032` rerun, and recurring once more at the final head `ebd29f454a` before passing 3/3 standalone there). `verify-client-domain-graph`'s 34 findings sit entirely outside every file this range touches. No fixture was hand-edited and no expectation was loosened to reach a clean run.
...
|---|---|---|
| `SCI-SETTINGS-SIDEBAR` | `deferred` | `verified` at `ebd29f454a5b86445190f004cc5f51cbfb6c3f1a` |
...
Independent review of this checkpoint at the intermediate SHA `e125ce0032` (via the docs-only closure commit `8c7ad720a8ba25949343af82cad340c8b56e5b31`, source-identical to `e125ce0032`) found a correctness defect in `ScienceSettingsCardController.save()` (`packages/client/ui-science/src/client/settings-card-controller.ts`): a save with two dirty fields judged landing as one all-or-nothing boolean across two independent `setPath` calls, so a Host that accepted one field's write and rejected the other left the accepted field's already-landed draft still staged, reported `restartRequired: false` despite a durable accepted change, and rendered `settings.saveFailed` copy claiming neither value was accepted. The fix — `ebd29f454a`, the binding final head — tracks landing per field: only a landed field's staged draft clears, `restartRequired` is set when any field lands, `failed` is set when any field does not land, and the two are now allowed to be true together; the `settings.saveFailed` copy (English and Chinese) no longer claims every changed value was rejected.

The independent reviewer implemented none of this range's changes. Beyond finding the defect above, the review reproduced the focused suites across `ui-science` and `ui-settings` (490 tests total), `typecheck`, `lint`, and the scoped per-file coverage recorded in the Verification matrix above; verified privacy by reading the changed code and by a repository-wide grep of every committed fixture and golden for the two real Conda prefixes used in acceptance, finding no leakage; verified the settings-card write coordinates against `attachRuntimeSettings` (`packages/science/science-runtime/src/settings.ts`) and the Host's `applyPathOp` (`packages/settings/settings/src/index.ts`); and confirmed bundle purity, registration/disposal correctness, and that the card holds a single state authority with no second `settings.describe` poll. It found exactly the one defect above, and no unresolved high-severity finding remains.
...
The ARIA golden `apps/web/tests/snapshots/plugin-config/section.expected.md` needed no refresh: it captures the Science card collapsed after a listing pass, never the `settings.saveFailed` text, and `plugin-config.e2e.ts` asserts no other string this fix changed. This narrower follow-up left the full `test:web` and `test:snapshot` lanes and real Conda acceptance for a subsequent full re-verification at this same fix commit; that re-verification is recorded in the Verification matrix above, which binds `ebd29f454a` as the final head. `test:e2e` and `pnpm install` remain outside this evidence's scope, as before.
