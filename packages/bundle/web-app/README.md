...
- **This bundle owns the `science-runtime` row id** — `cordis.patch.yml` mounts `@deepseek-ai/dsh-science-runtime/with-settings` under entry id `science-runtime` (an intentionally empty profile map; the settings card configures it). That entry and the package's root `@deepseek-ai/dsh-science-runtime` entry provide the same single-provider `ctx.scienceRuntime` Cordis service, so a deployment overriding this bundle's Runtime with explicit Cordis configuration must patch `science-runtime` by id rather than `insert` a second Runtime row — inserting one throws `service "scienceRuntime" has been registered` at load ([`dsh-science-runtime` README](../../science/science-runtime/README.md#settings-bound-entry)).
