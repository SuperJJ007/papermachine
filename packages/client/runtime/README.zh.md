...
公开的会话、列表、作用域和待答交互类型位于 `src/client/contract/`，由实现导入这些声明。待答交互接口提供响应投递，结算仍由 Session 负责。`agent-scope.ts` 与 `notifier.ts` 是对象层共享机制。

`Session.readScienceArtifact(versionId)` 调用 Session 作用域的 Host RPC，并把其 base64 载荷解码成字节。project store 坐标由 Host 解析和鉴权，而不是由 runtime 或调用方提供。预览与下载现在改为直接通过 `scienceArtifactUrl(sessionId, versionId)` 读取原始字节——这是 `GET /api/science/artifact/:sessionId/:versionId` 的可浏览器导航 URL，不带 base64 膨胀，也没有解码再编码这一趟——因此 `readScienceArtifact` 只为该 RPC 剩下的非浏览器消费者保留。`scienceArtifactUrl` 通过 `dsh-host-apiproxy` 导出的 `resolveApiOrigin` 相对当前页面 origin 拼接（与 RPC 传输自己的 POST/WebSocket 流量所用的同一条规则），绝不硬编码主机或端口——桌面端每次启动都随机选择 Host 端口，这一点是必需的。`Session.readScienceVersions(versionIds)` 一次调用批量读取最多 200 个已获会话 fold 授权版本的当前库事实（标题、说明、内容来源、生产者身份、health 等），未获授权或不存在的 id 被静默略过而不是让整次调用失败——见 `dsh-host-apiproxy` 的 `scienceVersions` RPC。生产者身份由 store 持有；runtime 只原样传输，不根据对话位置推断。`Session.readScienceLibrary()` 的返回类型带有 Host 响应一直就有的项目级 `health: ScienceLibraryHealth` 字段。
