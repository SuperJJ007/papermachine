...
A row's `copyable: false` is a separate, weaker refusal than `broken`: the preset is healthy, selectable, and can become the default, but its own metadata says a copy of it cannot bind or execute — `dsh-tool-science` binds its durable identity to the literal `science` preset, so a byte-for-byte copy would mount the same tools but never work. Only the duplicate button disables, with its own localized reason (`notCopyable`) distinct from `brokenNoCopy`; the two pickers show the row exactly like any other selectable preset.
