...
type TurnTailNodeViewProps = ChatNodeViewProps<'turn-tail'> & { collapsedCount: number }
  & PropsRenderSlots<'conversation.chat.turnTail' | 'conversation.chat.assistant-actions'>

/** Turn-local actions and feature tail over the Location index, independent of Assistant placement. */
export const TurnTailNodeView = memo(function TurnTailNodeView({
  node, openFile, forkAt, renderSlot, t, useChat, collapsedCount,
  ...
  const owner = { turn, seq: closing?.finalNode.seq ?? data.seq, openFile, collapsedCount }
  const tail = renderSlot('conversation.chat.turnTail', owner)
  if (closing === null) return tail
