...
Submission handlers can claim composer input before ordinary prompt transport. Registration is disposable; an unclaimed input follows the standard path. `openView` uses the registered controller to open the originating Session through `uiWorkspace.openSession`, select the Conversation main panel, and commit its view request and focus; a Session with no registered store throws. Event definitions can exclusively own one user source kind per target; duplicate ownership is rejected.
