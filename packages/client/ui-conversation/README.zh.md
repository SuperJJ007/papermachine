...
提交处理器可以在普通 prompt 传输之前接管输入框提交。注册可释放，未被接管的输入走标准路径。`openView` 经已注册的控制器调用 `uiWorkspace.openSession` 打开发起操作的会话、选中 Conversation 主面板，再提交该会话的视图请求及焦点；会话没有已注册 store 时抛错。事件定义可以独占某个 target 下的一种用户来源，重复所有权会被拒绝。
