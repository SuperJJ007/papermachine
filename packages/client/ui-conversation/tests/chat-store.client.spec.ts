...
describe('createChatStore', () => {
  it('init shape: empty selection/draft/view/detailsView', () => {
    const store = createChatStore().create()
    expect(store.store.getSnapshot()).toEqual({ selection: null, draft: '', view: null, inspect: null, detailsView: null })
  })
  ...
  it('actions cover the declared write set', () => {
    ...
    store.actions.setDetailsView('science')
    expect(store.store.getSnapshot().detailsView).toBe('science')
  })
