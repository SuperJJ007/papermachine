...
AppFrame 始终挂载会话栏和详情栏；已连接 Session 通过 `SessionProvider` 渲染，而详情栏对任何当前会话（含空白会话）都渲染会话作用域的 `details` slot，只有真正的欢迎页（完全没有当前会话）才回退到 `details.files`。布局 store 会把详情栏的宽度／开合偏好持久化到 `localStorage`（`dsh.layout.panels.v1`），跨刷新保留；侧边栏宽度以及窄视口断点字段（`narrow`、`narrowExpanded`）声明为 `transient`，每次都从 `init()` 值重新开始——侧边栏是导航 chrome，不是要保存的工作状态，窄视口字段则是随视口实时推导的值。打开详情栏时，会话栏与详情栏默认平分扣除侧边栏后的宽度。AppFrame 会跨越未选中状态保留最后一个非 blank 会话 id：返回同一会话时保留其宽度；选择不同会话时，详情栏会在绘制前关闭。会话 owner share 为空，侧边栏 owner share 只包含 `collapsed` 和 `width`；注册方通过标准钩子获取业务数据，并从各自的 inject 接口获取操作。
...
- **侧边栏几何信息是瞬时状态，详情栏几何信息会持久化**：重新加载会恢复侧边栏默认宽度，而详情栏的开合状态与拖动后的宽度会跨刷新保留（`dsh.layout.panels.v1`）；在不同会话 id 之间切换仍会关闭详情栏并忘记拖动后的宽度，未选中表面则以零宽度渲染详情栏，但不会修改几何信息。
