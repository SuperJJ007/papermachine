...
AppFrame 始终挂载会话栏和详情栏；已连接 Session 通过 `SessionProvider` 渲染，而详情栏对任何当前会话（含空白会话）都渲染会话作用域的 `details` slot，只有真正的欢迎页（完全没有当前会话）才回退到 `details.files`。布局 store 是瞬时状态，侧边栏以默认宽度启动，详情栏则保持关闭，且该 store 从不读写 `localStorage`。打开详情栏时，会话栏与详情栏默认平分扣除侧边栏后的宽度。AppFrame 会跨越未选中状态保留最后一个非 blank 会话 id：返回同一会话时保留其宽度；选择不同会话时，详情栏会在绘制前关闭。会话 owner share 为空，侧边栏 owner share 只包含 `collapsed` 和 `width`；注册方通过标准钩子获取业务数据，并从各自的 inject 接口获取操作。
