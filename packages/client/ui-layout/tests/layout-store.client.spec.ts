...
describe('createLayoutStore', () => {
  ...
  it('keeps live instances independent and restores only the saved rightbar width', () => {
    ...
    a.actions.selectPanel('temporary' as MainPanelId)
    a.actions.setViewportWidth(900)
    a.actions.toggleSidebar()
    a.actions.openRightbar(false, true)
    a.actions.closeRightbar()
    vi.stubGlobal('innerWidth', 1440)
    const restored = createLayoutStore().create()
    expect(restored.store.getSnapshot()).toEqual({
      panelInfo: { activePanelId: null },
      layoutInfo: {
        sidebar: 280, viewportWidth: 1440, narrowExpanded: false,
        rightbar: a.store.getSnapshot().layoutInfo.rightbar, rightbarShown: false,
        rightbarTrack: false, rightbarFullscreen: false, rightbarInstant: false,
      },
    })
    expect(JSON.parse(localStorage.getItem('dsh.layout.panels.v2')!)).toEqual({ rightbar: 864 })
  })

  it('discards the previous layout preference key', () => {
    localStorage.setItem('dsh.layout.panels.v1', JSON.stringify({ rightbar: 900, viewportWidth: 400 }))
    expect(createLayoutStore().create().getSnapshot().layoutInfo).toMatchObject({ rightbar: null, viewportWidth: 1920 })
  })

  it.each([null, [], {}, { rightbar: '600' }, { rightbar: 299 }, { rightbar: -1 }])('rejects invalid width preferences %j', (saved) => {
    localStorage.setItem('dsh.layout.panels.v2', JSON.stringify(saved))
    const error = vi.spyOn(console, 'error').mockImplementation(() => {})
    expect(createLayoutStore().create().getSnapshot().layoutInfo.rightbar).toBeNull()
    expect(error).toHaveBeenCalledOnce()
  })

  it('ignores unrelated persisted fields and accepts an uninitialized width', () => {
    localStorage.setItem('dsh.layout.panels.v2', JSON.stringify({ rightbar: null, panelInfo: { activePanelId: 'temporary' } }))
    expect(createLayoutStore().create().getSnapshot().panelInfo.activePanelId).toBeNull()
    expect(createLayoutStore().create().getSnapshot().layoutInfo.rightbar).toBeNull()
  })
