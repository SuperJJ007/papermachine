...
`FileDeliveryCard` and `FileDeliveryGroup` serve both explicit workspace deliveries and Science Turn-end artifacts. The card owns layout, sibling preview/action buttons, the optional menu, and focus return; the group owns the grid, empty-list handling, and local expansion state, requiring an owner-supplied `collapsedCount`. Callers supply titles, thumbnails, localized labels, and callbacks while retaining path resolution, Host requests, artifact identity, and version selection. Neither control reads files, accesses the network, or holds Cordis or domain data.
