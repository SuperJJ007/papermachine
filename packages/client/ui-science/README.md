...
Science artifact presentation metadata accumulates in authoritative Turn data. After the Assistant reply, one Turn-tail group renders one card per logical artifact and keeps only the highest version produced in that Turn. Cards show a thumbnail or media-type tile, a display name (this Turn's own kept version's curated title, or logical name when that title is empty), and version; activating a card opens that exact version in the right Sidebar. `annotate_artifact` remains a folded process cell and does not render an artifact card at the call site, so files appear once in the transcript. The group follows the [Chat disclosure configuration](../ui-chat/README.md#completed-turn-footer), using shared delivery cards and a group control that expands and collapses repeatedly. The title's count is always the Turn's total, never the visible slice, and the expanded/collapsed choice is component-local view state that resets on reload.

`science-artifacts` registers at order `20` in the `conversation.chat.turnTail` list, independently of workspace files, and returns `null` without artifacts. Science retains artifact/version identity, exact-version URLs, and authorized thumbnail loading. `ArtifactFileTile` maps media types to shared `FileTypeIcon` artwork without reading files or offering a native file menu.
...
Image failures belong to the displayed source URL. A different chart preview source replaces a failed image without reloading the saved version, and a detached image's late error cannot invalidate its replacement. Download notices and save-as forms belong to the selected Session and immutable version; switching versions clears those controls. An already requested download still completes against its original version URL.
