...
- **`conversation.composer.dock`**（`ScienceKernelStatus`） — composer 下方展示来自 `science` 投影 `kernels` 列表的逐语言最近生命周期记录（`started`/`exited`/`interrupted`）。这些历史记录不能证明进程当前存活；没有投影或没有内核记录时不渲染任何内容。
