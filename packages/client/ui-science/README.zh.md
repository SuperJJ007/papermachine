...
- **标签栏** — 固定的文件库主页 tab 之后是可关闭的 artifact tab 与 workspace file tab。点击会话记录 artifact 会打开其精确版本；关闭最后一个文档会选中文件库，主页 tab 不会消失。
