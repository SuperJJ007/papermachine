...
- **实时图表编辑面板从不挂载** — `ArtifactContent.tsx` 已经没有客户端可用的读路径来取 `ScienceChartState`（此前会话投影自己的 `chart` 字段携带它，T1/T2 产物权威性迁移把图表状态挪到了 store 一侧）。`previewChartOps` 也不能替代一次专门的读取：它会拒绝空的 ops 列表，所以没有"传零个 op 只为取当前状态"这条路。`ScienceChartEditPanel.tsx` 本身没变，也有完整的独立测试覆盖；直接的标题/图例/网格/字体编辑与元素级标注 targeting 在产品里暂不可用，直到后续任务补上一条读路径（属于 `packages/science/tool-science` 的 `edit-service.ts`，不在本包范围内）。区域选择 targeting（`RasterArtifact` 的拖拽手势）不受影响——它直接读 raster，不依赖可寻址的元素列表。
