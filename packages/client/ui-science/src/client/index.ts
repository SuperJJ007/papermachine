 ...
 * Details entry, browser half: registers the `science` locale dictionaries,
 * the two dedicated keyed toolview rows — `save_chart` and
 * `publish_outcome` — the Science settings card keyed on the
 * `science-runtime` namespace, the `conversation.session.header.actions`
 * entry that opens the routed Details column, and the `conversation.details.view`
 * entry (id `science`) that renders current Science state from the same
 * projection. The toolview rows are pure functions of the frozen
 * call/result slice, the loaded durable image bytes, and (for the Outcome
 * row) the live `science` session projection; the settings card owns its own
 * staging over the bound settings scope; the Details entry is a pure reader
 * of the same projection plus this package's own session-scoped attachment
 * loader (science-attachment-loader.ts).
 */
...
// conversation.details.view slots' declarations (same cross-plugin rule).
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** The dedicated chart/Outcome rows', settings card's, header action's, and Details entry's copy. */
    science: ScienceKey
  }
}
...
export function apply(ctx: ClientContext): void {
  ...
  // re-registration; the component reads the standard `t` seat instead.
