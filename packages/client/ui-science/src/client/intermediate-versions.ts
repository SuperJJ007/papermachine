 ...
 * later version of the same artifact shares both its authorizing turn and
 * its producing session. `turn` alone is a per-session counter — a
 * continuation session's turn 1 must never fold against a different
 * session's turn 1. A human-edit version (no authorizing turn) is never a
 * candidate.
 */
...
export interface ScienceIntermediateVersionFact {
  ...
  /** The session that produced this version; turn numbers are only comparable within the same session. */
