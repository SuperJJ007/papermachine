...
function setup(sessionsOverride?: unknown) {
  ...
  // Returned as its own plain local below (not read back via ctx.get(...).cancel)
  // so an assertion against it never trips the unbound-method lint rule that
  // a real interface-typed method reference would.
  ...
  return { ctx, slots, scienceEdits, conversationCancel }
}
