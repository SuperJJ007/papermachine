...
当前布局和标识计数器按会话持久化到 `dsh.sidebar.right.v2`；忽略 v1 偏好。恢复时先校验完整节点树、标签归属、活动引用、分栏比例、浮窗几何和标识计数器，再绑定资源实例。无效偏好保留可用的初始状态。撤销历史仅保留在当前页面，刷新后重新开始。
...
- **浏览器本地持久化。** 偏好仅属于当前浏览器；刷新恢复布局，不恢复撤销历史。每个会话恢复原生标签布局及展开状态。恢复的记录在渲染前绑定资源实例。`openResourceIn` 与 `openTabIn` 始终指向操作来源会话。
