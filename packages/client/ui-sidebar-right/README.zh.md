...
状态按会话保存在浏览器存储的 `dsh.sidebar.right.v2` 下，忽略 v1 偏好。切换会话保留各自布局；刷新在渲染前恢复原生记录及其资源实例。
...
- **浏览器本地持久化。** 存储的 JSON 必须是对象，但嵌套布局记录尚无完整结构校验；损坏的浏览器偏好可能导致恢复失败。每个会话恢复原生标签布局及展开状态。恢复的记录在渲染前绑定资源实例。`openResourceIn` 与 `openTabIn` 始终指向操作来源会话。
