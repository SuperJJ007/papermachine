...
Sidebar shell plugin: the brand row, New Session action, layout-owned collapse control, product-destination seat, scroll-aware region seat, and bottom-pinned Settings seat. Product plugins register stable navigation rows into `sidebar.destinations`; [ui-workspace](../ui-workspace/README.md) owns the Workspace and Session browser rendered into `sidebar.workspaces`. This package derives neither set of rows and owns no view preferences. Collapse into the layout-owned 56px rail remains presentation-local. Contract: the [slot system standard](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.md).
...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared brand, `sidebar.destinations`, `sidebar.workspaces`, and `sidebar.settings` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
