...
The owner payload is `ToolCallOwnerProps`: `callId`, `toolName`, the frozen `block`, optional `cwd` and `home`, and plain `openFile`/`inspect`/`loadImage`/`openDetailsView` callbacks — `openDetailsView(id)` selects a `ui-conversation` `conversation.details.view` entry and opens the Details column, the same capability its session-header action seat carries. Path summaries relativize to the session cwd first, then replace a leftover POSIX host home with `~`; `filePath` and Host open keep the authored filesystem path. The registration receives the normal session slot runtime share. It does not receive React nodes, Runtime services, or root/subcall knowledge.
