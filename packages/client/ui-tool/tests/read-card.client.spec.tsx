...
describe('GenericToolCard read body', () => {
  const ownerProps = (block: RunningToolCall | ToolResultNode): GenericToolCardProps => ({
    callId: 'c1', toolName: 'web_fetch', block, openFile: vi.fn(), loadImage: vi.fn(), openDetailsView: vi.fn(), t,
  })
  ...
  it('a non-read tool renders the bare row with no read card', () => {
    const view = render(<GenericToolCard {...({
      ...
      }), openFile: vi.fn(), loadImage: vi.fn(), openDetailsView: vi.fn(), t,
    })} />)
    toggleRow(view)
    expect(view.container.querySelector('[data-read]')).toBeNull()
  })
...
describe('ToolDetailsView Output section (read)', () => {
