...
`step()` reuses the exact `PromptAssembly` `agent/pre-step` produced; it never reassembles the system prompt. Before each `buildRequest()` attempt — the first and every retry — it re-renders that assembly's dynamic contexts and re-projects them through the same `RuntimeContextProjection` the pre-step candidate used, then appends a fresh runtime-context snapshot or clearing marker only when the Session no longer carries the value this step already published (a replacement between attempts, for example). The ordinary first attempt appends nothing further because the pre-step candidate already matches. Restoration commits before that attempt's messages are derived from the log, so a retried request is never missing durable context that a pressure compactor or the `agent/request-error` handler removed.
