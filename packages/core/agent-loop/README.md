...
`step()` reuses the exact `PromptAssembly` assembled before `agent/pre-step`; it never reassembles the system prompt. The final Enter batch is authoritative for a newly projected runtime-context candidate: a candidate removed by the waterfall is not restored, and the exact last owned message appended by that batch becomes the first-request value. When the current assembly already matched a retained snapshot, the loop keeps that exact previously accepted message as a fallback so pressure compaction performed inside the waterfall cannot remove it from the first request. After that fallback check, the loop captures the exact owned message retained on the live surface and restores only that value before a retry when `agent/request-error` removed it. This preserves deliberate Enter-batch filtering and replacement while keeping both pressure-compacted first attempts and request-error retries durably reconstructable without repeated provider I/O.
...
Accepted user messages, assistant messages, tool calls and results, injected context, and steering are logged and sent on later steps. Raw stream chunks, lifecycle boundaries, and other log-only events are excluded. A retried request within the same step observes the same restored runtime-context snapshot as the attempt before it, even when a compactor shadowed the earlier copy between attempts.
