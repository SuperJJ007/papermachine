...
`step()` 复用在 `agent/pre-step` 之前组装的确切 `PromptAssembly`，从不重新组装系统提示词。对于新投影出的 runtime-context candidate，最终 Enter batch 具有权威性：waterfall 移除的 candidate 不会被恢复，而该批次最后追加的精确 owned message 会成为首次请求的值。当当前 assembly 已与一个 retained snapshot 一致时，loop 会把此前已接受的精确 message 保留为 fallback，因此 waterfall 内的 pressure compaction 不能让它从首次请求中消失。Fallback 检查完成后，loop 会捕获 live surface 当时实际保留的精确 owned message；若 `agent/request-error` 将其移除，retry 只恢复这个值。这样既保留了有意的 Enter-batch filtering 与 replacement，也使 pressure-compacted first attempt 与 request-error retry 都无需重复 provider I/O 即可从 durable 记录重建。
...
已接纳的 user 消息、assistant 消息、工具调用与结果、注入上下文和 steering（中途引导）都会记录，并在后续步骤中发送。原始流分片、生命周期边界和其他仅写入日志的事件会被排除。同一步骤内的重试请求会看到与前一次尝试相同的、已恢复的运行时上下文快照，即使某个压缩器在两次尝试之间遮蔽了更早的副本。
