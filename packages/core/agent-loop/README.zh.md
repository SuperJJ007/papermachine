...
`step()` 复用 `agent/pre-step` 产出的确切 `PromptAssembly`，从不重新组装系统提示词。在每次 `buildRequest()` 尝试——包括首次和每次重试——之前，它都会重新渲染该 assembly 的动态上下文，并通过 pre-step 候选所用的同一个 `RuntimeContextProjection` 重新投影；仅当会话不再携带该步骤已发布的值时（例如两次尝试之间发生了替换），才会追加一份新的运行时上下文快照或清除标记。普通的首次尝试不会再追加任何内容，因为 pre-step 候选已经匹配。恢复操作会在该次尝试的消息从日志派生之前提交，因此重试请求永远不会缺失被压力型压缩器或 `agent/request-error` 处理器移除的持久上下文。
