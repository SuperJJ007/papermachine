...
- **执行世界坐标**：`executionWorld` 为 `'remote'`。`cwd` 和私有 `runtimeRoot` 来自共享所有者；可执行文件查找会验证绝对路径，或根据沙箱 PATH 加显式覆盖来解析裸名称，并与所有 subprocess 提供方一致地拒绝含分隔符的相对路径。
...
- **环境边界**：一次受信任的控制 shell 探测会从 passwd 条目解析沙箱用户的登录主目录，以 base64 ASCII 传输沙箱环境，再进行一次严格 UTF-8 解码。`environmentBase` 会在 `spec.env` 之前应用：`'scrubbed-parent'` 会移除环境中的 `DSH_*` 和形似凭据的名称（`*KEY*`、`*SECRET*`、`*TOKEN*`），而 `'empty'` 从不继承任何名称开始，因此被请求的程序只收到显式映射。提供方的控制进程可以保留其已文档化的传输环境，但该传输环境不得成为目标程序的环境。空名称、`=` 和违反 NUL 分帧规则的条目会在启动前被拒绝。在用户 profile 脚本运行前，此后的 E2B 命令 shell 与 PTY 登录 shell 会获得位于根目录下、全新随机生成的 `HOME`，并为每个被清理的环境变量名设置空值覆盖；之后，请求的 argv 会在不改变沙箱用户 umask 的前提下接收序列化环境。宿主环境变量绝不会隐式进入沙箱。私有环境文件在使用后会被删除；命令或终端设置失败时，会先删除其私有状态再拒绝。收集模式的宿主读取会根据所保留的精确字节切片盖上 `utf8Validity`。
...
- **`interrupt()` 是有文档记录的空操作**：远程引导只发布 `setsid` 进程组 leader 的 id，从不发布用户 argv 自身的 pid，因此本提供方没有目标能够匹配该 seam 仅限直接子进程的约定；为实际 argv 进程发布第二重身份的工作被延后。
