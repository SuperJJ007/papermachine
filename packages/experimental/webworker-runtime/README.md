...
The [built-bundle import sweep](tests/compile/transform-corpus-check.ts) uses an artifact-only TypeScript configuration so workspace imports resolve published JavaScript rather than source aliases. Its Dockkit exception accepts only Node’s unknown-extension error for the exact built Dockkit or StateDot stylesheet; source paths, other failures and unexpectedly successful exempt imports remain errors. See the [CI observation decision](../../../.agents/notes/implemented/testing/2026-09-08-ci-completion-observations.md).
