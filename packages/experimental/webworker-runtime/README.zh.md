...
[已构建 bundle 导入检查](tests/compile/transform-corpus-check.ts)使用仅面向产物的 TypeScript 配置，使工作区导入解析到发布的 JavaScript，而非源码别名。Dockkit 例外仅接受 Node 针对已构建 Dockkit 或 StateDot 样式表精确路径报告的未知扩展名错误；源码路径、其他失败以及意外成功的豁免导入仍然报错。参见 [CI 观察决策](../../../.agents/notes/implemented/testing/2026-09-08-ci-completion-observations.zh.md)。
