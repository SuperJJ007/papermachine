...
| `tool-fs/` | Model-facing `read`/`write`/`edit` tools AND the executor (reads via `ctx.fs`, owns read windowing, dispatches `fs/*`); preserves filesystem semantics for session-cwd-relative paths and advertises sandbox escalation fields when the mounted `ctx.fs` confines. Its `./read-only` subpath is a second independently loadable entry sharing the same config that registers only `read`/`read_image`. | (registers on `ctx.tools`) |
