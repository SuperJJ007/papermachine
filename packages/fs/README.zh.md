...
| `tool-fs/` | 面向模型的 `read`/`write`/`edit` 工具以及执行器（通过 `ctx.fs` 读取，拥有读取窗口逻辑，分派 `fs/*`）；为会话 cwd 相对路径保留文件系统语义，并在已挂载的 `ctx.fs` 实施约束时声明沙箱升级字段。其 `./read-only` 子路径是共享同一份配置的第二个可独立加载入口，只注册 `read`/`read_image`。 | （注册到 `ctx.tools`） |
