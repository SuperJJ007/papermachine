...
- **Console isolation is unavailable.** Under the restricted token, children created with `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` die during DLL initialization with `STATUS_DLL_INIT_FAILED` (`0xC0000142`). The POC tried to fix this by adding the console logon SID (`S-1-2-1`) to the restricting list; on Windows 11 26200 `CreateWellKnownSid(WinLocalLogonSid)` fails with `ERROR_INVALID_PARAMETER` (87), the correct `WinConsoleLogonSid` yields a valid `S-1-2-1` but the child still dies, and the POC's final revision removed both the SID and console isolation. Children share the runner’s console. A GUI-hosted runner first allocates and hides its own console, retaining inherited pipe/NUL handles; CLI and pseudoconsole runners preserve their existing console. The runner owns a new console until process exit. This also lets console-subsystem launchers create descendants without attempting console allocation under the restricted token. See the [console ownership decision](../../../.agents/notes/implemented/bug-fix/2026-09-09-gui-acl-runner-console.md).
...
- **Confined descendants cannot rely on creating pipes with default security attributes.** On Windows Server 2022, both libuv named-pipe stdio and `CreatePipe` fail when opening the writable endpoint under the restricted token. The default pipe security descriptor does not grant a restricting SID write access, even when that SID is present in the token default DACL. Python `subprocess.run(capture_output=True)` and `os.pipe()` therefore fail with access denied; other libraries that create pipes need separate compatibility validation. Descendants can inherit the runner’s existing stdio handles. The provider does not weaken token restrictions or grant ambient pipe write access to accommodate capture.
