...
- **控制台隔离不可用。** 在受限令牌下，以 `CREATE_NO_WINDOW` / `CREATE_NEW_CONSOLE` 创建的子进程在 DLL 初始化期间以 `STATUS_DLL_INIT_FAILED`（`0xC0000142`）死亡。POC 尝试把控制台登录 SID（`S-1-2-1`）加入 restricting 列表来修复；在 Windows 11 26200 上 `CreateWellKnownSid(WinLocalLogonSid)` 以 `ERROR_INVALID_PARAMETER`（87）失败，正确的 `WinConsoleLogonSid` 能产出合法 `S-1-2-1` 但子进程仍然死亡，POC 的最终修订同时移除了该 SID 与控制台隔离。子进程共享 runner 的控制台。由 GUI 启动的 runner 会先分配并隐藏自己的控制台，同时保留继承的管道/NUL 句柄；CLI 和伪控制台 runner 保留已有控制台。新控制台由 runner 持有到进程退出。这也让控制台子系统启动器创建后代时不必在受限令牌下分配控制台。详见[控制台所有权决策](../../../.agents/notes/implemented/bug-fix/2026-09-09-gui-acl-runner-console.zh.md)。
...
- **受限后代进程不能依赖使用默认安全属性创建管道。** 在 Windows Server 2022 上，libuv 命名管道 stdio 和 `CreatePipe` 都会在受限令牌下打开写入端时失败。默认管道安全描述符没有授予限制 SID 写权限，即使该 SID 已存在于令牌默认 DACL 中。Python 的 `subprocess.run(capture_output=True)` 和 `os.pipe()` 因此会报访问被拒；其他创建管道的库需要单独验证兼容性。后代进程可以继承 runner 已有的 stdio 句柄。Provider 不会为支持输出捕获而放宽令牌限制或授予环境管道写权限。
