...
export function ensureRunnerConsole(api: Win32Bindings): void {
  // Windowless consoles still have a code page; HWND presence is not attachment.
  ...
  if (api.getConsoleCP() === 0) throw new Error('AllocConsole succeeded without an associated console')
  const window = api.getConsoleWindow()
  // ShowWindow returns the previous visibility, not a success flag.
  if (!isNullPtr(window)) api.showWindow(window, 0)
