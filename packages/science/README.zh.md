...
| 包 | 职责 | ctx 键 |
|---|---|---|
| [`science-session/`](science-session/README.md) | 拥有六个 Science Session 事件、严格 Host replay、pre-commit invariant、客户端安全 projection 与 artifact 附件提取。 | 组合时注册到 `ctx.sessionProjections` / `ctx.sessionAttachments` |
| [`science-runtime/`](science-runtime/README.md) | Host-local Conda Runtime：environment binding、Python/R execution、私有 scratch、run 写出文件的自动捕获，以及纯元数据的 artifact 策展。 | `ctx.scienceRuntime` |
