...
- **Prefix 漂移按每次 run 检测，而非按每次 import 检测** — 每次 `startRun` 之前都会核验 `conda-meta/history` 并在检测到变更时自动重新绑定环境并重启内核；若某内核在 prefix 变动前已经 import 了某模块，该旧模块会保留在内存中，直到下一次针对该语言的 run 触发 `environment-rebound`。
