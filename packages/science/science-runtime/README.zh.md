...
`annotateArtifact({ session, logicalName, version, title, caption, toolCallId, requestHeaderSeq, signal })` 针对当前 live Science projection 解析所命名逻辑 artifact 的精确 `version`(省略时取其最新版本)，并追加一条新的 `version_annotations` 行(`actor: 'model'`，携带本次调用的 `sessionId`/`toolCallId`/`requestHeaderSeq`)记录传入的 `title`/`caption`——store 的内容寻址引用、producer 字段与 `created_at` 均不改变。策展是加在 session 已持有的内容之上的元数据，因此它绝不会推进读者所看到的版本号，且省略 `caption` 会清空此前的值(每次调用对两个字段都是权威的)。它从不读取文件系统、从不从 project artifact store 读取 blob 字节，也从不公开 Host path；除共享的 pre-publication 失败外，当 `logicalName` 或其命名的 `version` 在本 session 中不存在时拒绝并抛出 `ARTIFACT_NOT_FOUND`，当该 version 的 store 行属于直接人工样式编辑(`content_origin: 'human-edit'`)时拒绝并抛出 `ARTIFACT_NOT_CURATABLE`——策展它要么抹去直接编辑的判别标记，要么冒认该编辑从未拥有过的模型授权，因此其消息会引导调用方改用一次新的 run 或 viewer 自身的样式编辑器——当所给 `toolCallId` 已经为本会话 projection 已知的某个逻辑 artifact 的既往一次 annotation 授权过时，拒绝并抛出 `ARTIFACT_ANNOTATE_TOOL_CALL_REUSED`,该检查针对每个这类 artifact 在 store 中的当前 `version_annotations` 行(持久、可在重启后成立;被后续 annotation 取代的 `toolCallId` 在此检查下不再可见,本会话从未加载过的 artifact 也不在扫描范围内)。
