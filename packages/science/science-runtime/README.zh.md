...
- **持久化 kernel spawn confinement 尚未读取所配置的 `minimumEnforcement`** — `execution.ts` 的 `confineInterpreterArgv` 接受一个 `minimumEnforcement` 参数，并为其唯一调用方(`kernel-process.ts` 的 `KernelProcess.start`)默认取 `'full'`，该调用方尚未把已解析的 Config 值传进来。interpreter probe confinement(`environment.ts` 的 `confineProbe`)如今已完整接入所配置的值。把这个值接进 kernel spawn 属于 win32 kernel-transport 那部分工作的范围，该工作也会移除 `KERNEL_UNSUPPORTED_PLATFORM`，因为只有当 kernel execution 本身能在该平台上运行时，降低最低要求才有意义。
