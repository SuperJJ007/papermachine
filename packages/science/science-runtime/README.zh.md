...
Runtime 在某个 Host 生命周期内第一次由 `sessionProject` 解析出某个 project id 时,触发一次全 project 的 store ↔ session 对账——`dsh-science-artifact-store` 的 `reconcile.ts` 算法,把库里的 version 行拿去和该 project 自己 session 日志里的每条 `science/artifact-saved` 事件比对——对同一个 project id 之后不会再跑第二次。这次对账是"发起后不管"的:它从不阻塞或拖慢触发它的那次 Science 操作,过程中任何一处失败(列出 session 日志、读某一条、或库自身的对账调用)都通过 `ctx.logger.warn` 记录,然后就此吞掉,不会向外传播。没有挂载 `sessionPersistence` 服务的部署(本 Runtime 自己的测试 harness,以及一些最小化的组装)会直接跳过整次对账,而不是把服务缺失当作错误。

`collectProjectArtifactEvents`(`reconcile-trigger.ts`)是本 Runtime 唯一一处读取自己存活 Session 之外的 durable session 日志的地方:它列出每一个 header `cwd` 解析后等于该 project 规范工作区路径的 session,通过 `SessionPersistence.inspect()` 逐个读取(读到的是原始、未解码的事件——绕过 `dsh-science-session` 的 codec,这样库的兜底重建所需要的字段就不会被一个为更瘦的存活形态而生的 codec 悄悄剥掉),并把它们的 `science/artifact-saved` 事件按 `versionId` 归并(后写覆盖)。读不了的 session 日志或格式不对的事件会被跳过并记一条 warning,而不是让整次遍历失败;`reconcileMaxSessions` 限定一次调用最多读多少个匹配的 session,超出的部分报告为 `truncated` 而不是全部读完。归并出的事件集会被交给 `ScienceArtifactStore.reconcileProject`,它从不写 session 日志——六种情况的分类见该包的 README。
