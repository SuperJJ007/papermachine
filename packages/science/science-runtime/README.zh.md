...
- **跨 session store 延续没有 chart 状态** — project artifact store 持久化 PNG 字节与版本元数据，不持久化归 session 所有的 `chart` 投影。一个 session 若延续由另一 session 首创的逻辑 artifact，无法只从 store 恢复先前版本的可寻址状态。
