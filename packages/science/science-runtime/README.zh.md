...
解析出已 applied 的 environment 之后，`startRun` 还会检查它所依赖的共享 prefix 自本会话自身绑定上次观测以来是否发生了漂移(#15:另一个会话调用 `installPackages` 会写入同一个共享 prefix，却从不触碰本会话自己的 `science/environment-bound` 历史)：对每个已声明且 `available` 的语言绑定，它会重新读取 `<canonicalPrefix>/conda-meta/history` 并把其 SHA-256 与绑定中记录的 `condaHistorySha256` 比较——每个已声明语言只需一次小文件读取，不起探针，不启动 confined subprocess，且这项检查每次 `startRun` 都会执行。一旦摘要不再匹配——包括彻底读不到的情形(文件缺失、被替换成目录或符号链接、或任何其它读取失败——一律视为漂移，绝不能当作"未漂移")——就会走下文 `installPackages` 所用的同一条重新观测/追加路径重新观测整个 profile，且只有在重新观测结果确实不同时才追加一条全新的整值 `science/environment-bound` revision；`startRun` 随后针对这条全新 revision 规划并执行该次 run，仍在服务旧 revision 的 kernel 也会像本会话自己调用 `installPackages` 之后一样重启(`environment-rebound`，见上文)。真正昂贵的部分——两种已声明语言各自的 version/UTF-8/package-inventory 探针——只在真正检测到漂移时才会运行，而非每次调用都跑一遍。若重新观测结果不是完全 `applied`，则绝不会追加：`bindEnvironment` 无法在会话跑过第一次 run 之后重新绑定它，因此在这里追加一条 `invalid` revision 会让该会话永久搁浅；`startRun` 转而只拒绝这一次调用，报 `ENVIRONMENT_NOT_READY`，并保留会话最后一次已知良好的绑定，好让针对已恢复 prefix 的后续调用仍能通过。

kernel 是 Runtime 作为包资产随包发布的一个小驱动脚本——`kernel_python.py`(只用 Python 标准库)或 `kernel_r.R`(只用 base R，不用 `jsonlite`，与 R package-inventory probe 自身的 TSV 选择一致)——因为无法假定用户配置的 prefix 内含任何 notebook-kernel package，而 Runtime 也从不向 prefix 内安装东西。host 向 kernel 的 stdin 写入单行请求，并从一个由 `selectKernelTransportKind` 按 kernel 逐个选定的 response-channel transport(`kernel-transport.ts` 中一个可插拔的 `KernelResponseTransport` seam)读取单行响应：在 darwin 与 linux 上，是它在 kernel 私有 scratch 中创建的一个 FIFO，通过受管 POSIX `cat` 子进程转发，使空闲读取不占用 Host 文件系统 worker；在 win32 上，是一个监听在 `127.0.0.1`、绑定到临时端口的 loopback TCP listener，持续接受连接，直到有一个连接给出为这个 kernel 专门生成的 32 位十六进制随机 token 为止——token 错误或缺失、或者以其它方式握手失败的连接会被销毁，而 listener 会继续接受后续连接，直到 kernel 启动 deadline 到期或 kernel process 自己退出；只有给出正确 token 的那个连接会停掉 listener，此后再出现的连接尝试才会被拒绝。两种 transport 交给 `KernelProcess` 的东西完全一样：一个携带相同 wire-protocol frame 的可读字节流，以及一个"已结束"/"已出错"信号；frame parser 与 kernel 状态机从不关心是哪种 transport 产生了它们。win32 之所以放弃 named pipe，是因为 Windows ACL sandbox 可能不会把 Host 创建的 named pipe 的写权限授予受限 token 的 kernel process，而 loopback socket 根本不是一种可被 ACL 保护的对象。一次 run 的 stdout/stderr 会写入请求所命名的 per-run 捕获文件，而不是 kernel 自身的 stdio。Python 在 file-descriptor 级别重定向一次 run 的输出(`os.dup2`)，因此 C 扩展与子进程的写入都会被正确归属到对应 run。R 只在 `sink()` 级别捕获：R 层面的 `print`/`cat`/`message` 输出会被正确归属，但 C 层面或子进程的写入会绕过 `sink()`，落入 kernel 自身长生命周期、有界的 stdout/stderr collector，永远不会归属回任何一次 run(见"已知限制")。当 driver 检测到自己无法完整恢复或归属某次 run 的输出捕获时，该 run 的 terminal event 会携带 `outputDegraded: true`；该 run 自身的 result 依然成立，只是其捕获的 tail 可能不完整。

已接受的 TCP socket 在排队、token 解析以及响应通道移交后都保留错误处理。启动失败清理期间的连接重置会关闭该连接，不会作为 Host 未捕获异常逃逸；token 和帧消费者保留原有的错误分类。

kernel 会因一组封闭的原因之一结束，通常作为 `science/kernel-state`(`state: 'exited'`)追加：`idle`(自 kernel 上一次完成的 run 起 `kernelIdleTimeoutMs` 已过去，其间再无 run——在 run 进行中时被 disarm)、`session-end`(所属 Session detach——这次追加的目标是已经 detach 的内存中 log，因此永远不会被发布或 durable 持久化；replay 会改为从 `session/end-seed` 把该 kernel 恢复为 `interrupted`，见"已知限制")、`environment-rebound`(该 kernel 语言存在更新的 applied environment revision；该语言的下一次 run 会先结束这个过时的 kernel，再针对新 revision 启动一个全新的)、`run-escalation`(一次被中断的 run 使 kernel 自身状态无法证明，见下文)、`protocol`(driver 违反了 wire protocol——一个无法解析的 frame、response channel 的意外结束，或比 `kernelStartTimeoutMs` 更慢的 `READY` handshake)，以及 `crash`(kernel process 在未观察到协议违反的情况下无指令退出)。一次其 kernel 在运行中无指令死亡的 run，会以 `failureCode: 'KERNEL_DIED'` 结算。每个 run 的 terminal event，以及 `get_science_state`，都会命名执行它的 `kernelEpoch`，因此模型与 Reviewer 都能准确判断哪些 run 共享了内存。 响应通道的 TCP reset 与 EOF 共用有界的进程退出观测：内核进程已退出时归类为 crash，进程仍存活但通道已关闭时归类为 protocol。主动关闭期间的流错误不会覆盖 commanded 退出原因。

取消或超时一次正在进行的 kernel execution 时，会先尝试中断、再考虑杀死：Runtime 向 kernel process 发送 `SIGINT`，并等待一个固定的宽限窗口，看 kernel 是否回应它捕获了这次中断。如果是，该 run 的 terminal 照常是 `cancelled`/`timed-out`，而 kernel 状态完好地继续运行——该语言的下一次 run 仍然能看到更早的每一个变量。如果 kernel 反而正常结束了这次 run(它自己的代码抢在中断之前完成，或者捕获并吞掉了中断)，或者没有在宽限窗口内回应，kernel 自身的状态就不再可信：Runtime 会在该 run 结算之后结束它(`run-escalation`)，该语言的下一次 run 会启动一个全新的 kernel。无论哪种情况，该 run 自身的 terminal status 都只由触发 abort 的原因决定(取消还是超时)，绝不会因 kernel 的命运而改写。在 win32 上，`interrupt()` 是一个有文档记录的空操作(这个 Runtime 在 win32 上没有等价于 `SIGINT` 的手段可发)：每一次被取消或超时的 win32 run 都会直接落入上文的"未回应"路径，永远升级到 `run-escalation`，永远丢失 kernel 的内存状态——见"已知限制"。

### 自动捕获

自动捕获会把 artifact store 写入期间提交的全部事件纳入 session projection，包括 kernel 退出与用户修改标题。并发事件不会使捕获回执失效。
...
`installPackages({ session, language, packages, signal })` 要求一对已配置的 `micromambaPath`/`installChannels`(否则以 `ScienceRuntimeError('INSTALLER_NOT_CONFIGURED', …)` 拒绝)，并要求 `language` 有一个已 applied 的 environment 且没有正在进行的 run(`ENVIRONMENT_NOT_READY`/`RUNTIME_BUSY`，与 `startRun` 自身的孤儿检查一致)。它会在一个以 prefix 自身为 root 的 `workspace-write` sandbox 策略下运行一次 direct、unconfined 的 `micromamba install --yes --no-rc --prefix <canonicalPrefix> --override-channels --channel <一个 URL> <packages>`——这是本 Runtime 中唯一一处授予 prefix 写权限而非要求其只读的 confinement 站点，因为安装正是唯一一种整个目的就是写入该 prefix 的操作——并由 `installTimeoutMs` 而非通用的 `timeoutMs` 限定时限。这次 confinement 同样要求报告级别不低于所配置的 `minimumEnforcement`——与一次 interpreter probe 或持久化 kernel spawn 要求相同(否则以 `ScienceRuntimeError('CONFINEMENT_UNAVAILABLE', …)` 拒绝，消息同时点出实际报告与所要求的级别)；因此把 `minimumEnforcement` 降为 `'partial'` 的 win32 桌面部署，其 `installPackages` 同样在 partial enforcement 下运行，详见下方“限制与环境”一节。每个已配置的 channel URL 都会被依次当作一次完整、独立的调用来尝试；只有 `'failed'` 的一次尝试才会尝试下一个 URL，且任何一次调用都绝不会被赋予一个以上的 channel(一份 conda channel 列表意味着"搜索全部"，而非"依次回退"，把多个来源混进一次求解可能会从不同镜像里拉取包，拼出一个不一致的安装结果)。一个以 `0` 退出的 micromamba 进程会被分类为 `'success'`，即便截止时间先一步单独触发——因为在 `installTimeoutMs` 到期后 SIGTERM 宽限窗口内落定的求解已经写好了目标环境，所以 `'timed-out'` 只留给未能成功完成的进程。只有在成功时，Runtime 才会像 `bindEnvironment` 一样重新观测整个 profile；若重新观测结果与该会话当前任一语言的绑定不同，就会追加一个全新的整值 `science/environment-bound` revision——如果重新观测发现能力失败就是 `invalid` revision，否则是 `applied`——而若重新观测结果与当前绑定及其 package inventory 完全一致(请求的每个包早已存在)，则不追加任何 revision，返回既有环境并附带 `environmentChanged: false`，因此一次冗余重试从不会推进 revision 或在环境毫无变化时重启 kernel。这次调用本身从不重启任何 kernel:一次追加的 revision 只会在该语言下一次 `startRun` 时生效，那次调用会发现 revision 不一致，先结束过时的 kernel(`environment-rebound`，见上文)再启动一个全新的——因此调用方应当预期内存中的 kernel 状态会在那次 run 才消失，而不是这次。一次失败、被取消或超时的安装会返回相应分类连同有界的 output tail，且不会追加任何 environment revision，durable projection 与每个存活 kernel 都会保持原样。绑定到同一共享 profile 的另一个会话不会直接看到这次调用追加的 revision，但那个会话自己下一次 `startRun` 现在会自行捕捉到由此产生的 prefix 漂移并自我重新绑定(参见上文 startRun 自身关于漂移检测的段落)。
...
fake-prefix 测试覆盖 Python-only、R-only、shared 与 distinct prefix；严格配置，包括持久化 kernel 的 idle 与 spawn-to-READY 截止时限边界；稳定与漂移观测；无效 UTF-8 probe byte；两种语言的 package-inventory 解析、排序、entry/byte 上限截断与 probe 失败处理；scratch ownership；direct argv；空环境；output 上限；terminal 分类；取消；超时；detachment；同 ID quarantine；Loader 组合；以及 live/cold replay。kernel 相关机制拥有自己的一组测试，针对一个说 kernel wire protocol 的 fake driver，并通过真实的 subprocess-local 与 sandbox-local provider 组装：driver 资产解析(覆盖 source 与 built 两种执行方式)；kernel-process 的 handshake、连续多次 run、DONE 路由、READY 超时、协议乱码、run 中途的无指令退出、中断后存活、中断后升级、EXIT teardown 与 response-channel 清理；loopback TCP transport 拥有自己的一套测试，针对一个真实的 `node:net` client(token 接受；错误 token、过早 EOF、超限 token 行或出错的连接会被销毁，而 listener 会继续接受后续正确 token 的连接，包括一个在 `connect()` 被调用之前就已排队的连接；一次发生在 `connect()` 之前的 listener 故障会被立即观测到；一个已有合法连接落定后再出现的第二次连接被拒绝；EOF 分类；以及 listener teardown 会销毁任何仍在排队的连接)，另有一套 real-driver 测试强制使用 TCP transport(不 mock `process.platform`)、运行随包发布的 `kernel_python.py`/`kernel_r.R` 走完整的 RUN/DONE/EXIT 周期,针对本机自身已绑定或从 PATH 解析出的 interpreter，任一语言不存在时自行跳过；以及 kernel-set 的 epoch 分配与跨重启单调性、idle 过期与活动重置、Python 与 R kernel 共存、environment-rebound 重新 spawn、同 ID quarantine，以及 detach/dispose teardown。一套专门的自动捕获测试覆盖新文件、变更文件、字节相同的重跑(跳过)、超限文件、per-run 与 per-session 上限、dotfile/扩展名排除、对失败 run 的捕获、project artifact store 拒绝写入、一次不致命的内部捕获失败、不同会话各自创建同名产物（不同 artifactId、从 v1 开始、原产物 latest 不变），以及 Host 重启后的持久化读取。一个专门测试将 `bindingFingerprint` 与 package inventory 的独立性钉住：对同一静态 identity 重新绑定并观测到不同的 inventory 时，只会改变 `packagesSha256`，不会改变 `bindingFingerprint`。只用 lstat 的 prefix manifest 记录相对路径、类型、symlink target、mode、size、mtime/ctime nanoseconds 与 regular-file digest，且不使用 atime；前后 diff 为空才表示 prefix 未改变。`installPackages` 有自己的一套测试，覆盖 package-spec 校验、argv 构造(每次调用绝不超过一个 channel)、写权限 confinement 的分歧点、install-scratch 生命周期与 subprocess 分类——包括一次落在截止时间 SIGTERM 宽限窗口内的零退出码落定被分类为 `'success'` 而非 `'timed-out'`；此外还有编排层面的覆盖，涵盖每一种 pre-publication 拒绝、单 channel 成功与失败、R-only 重新观测、一次无效的重新观测 revision、scratch 清理失败日志、整段式 channel 回退(第一个 channel 失败后尝试下一个、所有 channel 都失败、以及一次被取消的进行中尝试不会再尝试任何 channel)、一次确实改变了 inventory 的成功安装追加一个全新 revision，以及一次冗余的成功安装重新观测到相同 inventory、不追加任何 revision 并报告 `environmentChanged: false`。`installTimeoutMs` 自身的配置解析边界(默认值、最小值、最大值，以及与 `timeoutMs` 自身独立最大值的相互独立性)与其他已解析字段一样有专门的覆盖。`minimumEnforcement` 的配置解析默认值与对非法值的拒绝、`confineWithEnforcement` 的 full/partial 接受矩阵(包括 `CONFINEMENT_UNAVAILABLE` 消息同时点出实际报告与所要求的级别)，以及一个配置为 `'partial'` 的最低要求端到端接受一个报告 `'partial'` 的 sandbox(通过 `bindEnvironment`)，均有专门的覆盖；`KernelProcess.start` 自身的 confinement 有对称的一对用例——传入 `minimumEnforcement: 'full'` 时拒绝一个报告 `'partial'` 的 sandbox，传入 `'partial'` 时接受。`KernelSet` 把自己配置的值转发进这个参数(`acquire` 的 spawn 路径与 `startIsolated`)在更上一层也有专门覆盖：每一处转发都有一个在缺少该行转发时会失败的专门测试。另有一个端到端用例针对一个 `Config.minimumEnforcement: 'partial'` 的 runtime 与一个报告 `'partial'` 的 sandbox 发起一次真实的 `startRun` kernel spawn，覆盖同一条链路穿过 `ScienceRuntime` 本身，而不只是穿过一个直接构造的 `KernelSet`。`confineInstallArgv` 自身的 full/partial 接受矩阵有相同的专门覆盖，包括完全相同形状的 `CONFINEMENT_UNAVAILABLE` 消息；`installPackages` 把 `this.minimumEnforcement` 转发进该调用同样在更上一层有一对端到端用例——配置为 `'partial'` 的最低要求对一个报告 `'partial'` 的 sandbox 会成功，而 sandbox 报告的级别低于所配置最低值时会拒绝——与 `KernelSet` 自己那对用例采用相同的转发回归覆盖形状。一套专门的共享 prefix 漂移测试针对 Python 与 R 都驱动一次真实的 `startRun`，并通过真实修改磁盘上的 `conda-meta/history`(而非手动追加一条 durable event)来触发漂移：漂移的 prefix 会重新绑定并重启过时的 kernel；未改变的 prefix 不追加任何内容且保持同一个 kernel epoch；漂移后已不可用的 prefix 以 `ENVIRONMENT_NOT_READY` 拒绝且不追加任何 revision，等 prefix 恢复后该会话仍能再次运行；history 被替换成目录时走的是同一条重新观测路径，而非一个未分类的异常。
...
- **Prefix 漂移检测只覆盖 conda 事务，且按每次 run 检测而非按每次 import 检测** — `startRun` 的摘要比对(见上文)读取的是 `conda-meta/history`，因此能捕捉 `install_science_packages`/micromamba 写入共享 prefix 的包，但捕捉不到用户在产品之外手动装进同一 prefix 的包(直接 `pip install` 写的是 `site-packages`，从不写 `conda-meta/history`)。本产品自身的 pip / `install.packages()` 路径无论如何都不会碰共享 prefix：kernel 内 inline 安装落在该 kernel 私有的 `PYTHONUSERBASE`/`R_LIBS_USER` 里，从来就不是这个缺口的来源。检测也是按每次 `startRun` 调用执行一次，而非按 run 内部每次 import 执行：一次 run 进行期间发生的共享 prefix 变更，要等到该 run 之后的下一次 `startRun` 才会被捕捉，而不是当前这次 run。
