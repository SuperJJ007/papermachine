...
Runtime 在 `sessionProject` 解析出 project id 时触发一次有界的全 project store ↔ session 对账尝试，即运行 `dsh-science-artifact-store` 的 `reconcile.ts` 算法，把存储里的 version 行与该 project 的 `science/artifact-saved` 事件比对。只有完整事件收集以及未截断、无错误的存储对账运行成功，才会在当前 Host 生命周期内抑制该 project 的后续尝试。列举失败、日志不可读、目标事件格式错误、session 或 version 截断、存储条目错误，以及被拒绝的存储调用都会让该 project 在 `reconcileRetryDelayMs` 之后被另一个 session 解析时继续符合重试条件；系统不会创建后台 timer 或重试循环。每次尝试都是 fire-and-forget：它从不阻塞或拖慢触发它的 Science 操作，每项失败都通过 `ctx.logger.warn` 记录并吞掉，不向外传播。没有挂载 `sessionPersistence` 服务的部署会直接跳过尝试，而不是把服务缺失当作错误。

`collectProjectArtifactEvents`（`reconcile-trigger.ts`）是本 Runtime 唯一一处读取自己存活 Session 之外的持久 session 日志的地方。它列出每一个 header `cwd` 解析后等于该 project 规范工作区路径的 session，通过 `SessionPersistence.inspect()` 逐个读取（读到的是原始、未解码的事件，绕过 `dsh-science-session` 的 codec，因此存储兜底重建所需的字段不会被面向更精简 live 数据的 codec 静默剥除），并把其中的 `science/artifact-saved` 事件按 `versionId` 归并（后写覆盖）。不可读的 session 日志或格式错误的目标事件会被跳过并记录 warning，而不是让整次遍历失败，但结果会标记为不完整；列举失败同样不完整，匹配 session 超过 `reconcileMaxSessions` 时则同时标记不完整和 `truncated`。归并后的事件集与完整性标志交给 `ScienceArtifactStore.reconcileProject`，该方法从不写 session 日志。不完整事件集无法证明缺少事件的存储行是 orphan，因此存储把该行分类为 `unverified`，保留其既有 orphan 标志，同时继续检查内容冲突、重建悬空事件、比较元数据并刷新 `missingContent`。七种情况的分类见该包的 README。
