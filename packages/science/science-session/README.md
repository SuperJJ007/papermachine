...
Six `science/*` Session events, each `version: 1`, lossless JSON, carrying a complete domain value rather than a patch, and required on read (never `ignorable`): `science/mode-bound`, `science/environment-bound`, `science/run-started`, `science/run-finished`, `science/artifact-saved`, `science/outcome-published`. `science/mode-bound` is legal once, only for a Session whose `agentPreset` is `science`, and before the first Science-preset `step/start`, `request/header`, or `tool/call` fact. `science/artifact-saved` generalizes the durable model beyond charts: an artifact version carries an always-populated `title`, an optional `caption`, an `origin` of `'auto'` (unattended capture) or `'model'` (curated), and its complete `ImageAttachmentRef`. Outcomes retain non-empty references to prior successful runs, exact artifact versions, and/or message facts.

## Strict fold and invariant

`replayScience(events)` deterministically replays a complete contiguous log into the complete Host-side `ScienceProjection`, or `null` before a valid mode binding. The fold rejects discontinuous sequences, malformed values, invalid transitions, forward provenance (a `requestHeaderSeq`/`toolCallId` must name the latest post-mode fact of its kind), reused or settled tool calls, non-monotonic revisions or times, and foreign evidence. A curation-only re-save that repeats an identical `attachment` with a changed `title`, `caption`, or `origin` still commits the next contiguous version — the fold applies no content-hash dedup of its own; a caller that wants to skip an unchanged file compares content hashes itself before appending. `session/end-seed` alone derives `interrupted` for an unmatched running run; no synthetic Science terminal event is appended. The package-owned invariant (`./invariant`) applies the same applicability rule and strict fold before every commit, so a rejected candidate appends nothing to the durable log.
