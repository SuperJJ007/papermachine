...
- **`science/kernel-state` 尚未被投影。** 它在严格的 Host 重放中会像其它每一个 required 事件一样被 fold 与校验，但 `ScienceProjection`/`ScienceClientProjection` 尚未携带 kernel 事实。可选的增量 `ctx.sessionProjections` 缓存目前也不会在被缓存的各个步骤之间保留 kernel fold 状态：`decodeScienceProjectionFold` 每次调用都会将其置空，而不是从持久化 JSON 中读取；因此单靠这个缓存会静默丢弃 kernel-state 事件——持久化日志、`foldScience`/`replayScience` 以及 pre-commit invariant 均不受影响。
