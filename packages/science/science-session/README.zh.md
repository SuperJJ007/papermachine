...
Artifact parent 与其它来源证明一样，只能指向该 session 自身 log 中先前已提交的事实；run input 则不尽然——只有当该 session 自身的 fold 已经记录过同一个 artifactId 的某个版本时，才要求它指向一个先前已提交的事实,该 session 从未记录过的 artifactId 会被原样接受，信任实时 Runtime 已在该引用事件提交前对照所属 project 的 artifact store 校验过它(同一 project 中第二个 session 读取第一个 session 的 artifact)。package invariant 在 commit 前通过同一个严格 fold 拒绝缺失或前向引用。
...
- **`editBaselines`/artifact parent 血缘只按同一 session 校验；run input 不是。** 当该 session 自身的 fold 从未记录过所引用的 artifactId 时，run input 会在没有本地匹配的情况下被接受——实时的 `dsh-science-runtime` 已经在该引用事件提交之前对照所属 project 的 artifact store 校验过它，纯 replay 会信任这个已经校验过的事实,而不是重新推导它(同一 project 中第二个 session 读取第一个 session 的 artifact)。若该 session 的 fold 已经记录过某个 artifactId,但版本号不对,仍会被拒绝——这一点仅凭 log 本身就能识破。`artifact.parent` 引用(edit baseline、直接编辑血缘)仍只依据该 session 自身的 fold 解析；指名另一个 session 产出的版本仍会被拒绝，因为要接受它,还需要让上文的人工编辑不变量(parent 的 logical name、media type、environment provenance)针对一个本 fold 看不见的版本去校验。
