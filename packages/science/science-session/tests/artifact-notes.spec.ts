...
describe('user-only artifact-note projection', () => {
  it('retains store artifact identity, visible version, text, time, and add-event sequence', () => {
  ...
  it('removes only the addressed active note and ignores unrelated Session events', () => {
    const artifact1 = ScienceArtifactId('artifact-1')
    const artifact2 = ScienceArtifactId('artifact-2')
    const state = [
      { seq: 7, artifactId: artifact1, version: 1, text: 'first', createdAt: 100 },
      { seq: 8, artifactId: artifact2, version: 2, text: 'second', createdAt: 200 },
    ]
  ...
  it('rejects empty, overlong, and structurally extended note projection values', () => {
    const base = { seq: 1, artifactId: 'artifact-1', version: 1, text: 'note', createdAt: 1 }
