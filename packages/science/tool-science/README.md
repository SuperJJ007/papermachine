...
`scienceEdits.applyChartOps` forwards an exact `{ artifactId, version, ops }` request and cancellation signal to `ctx.scienceRuntime.applyChartEdit`. Its receipt names the committed `origin: 'human-edit'` version and any indexed partial failures. It maps stale, unaddressable, and invalid or wholly unresolved operations to `CHART_STALE`, `CHART_NOT_ADDRESSABLE`, and `CHART_OP_INVALID`; the Runtime remains the only validator. Model-facing state and artifact receipts do not summarize cumulative direct-edit operations — the underlying live-figure-object state lives only in the project artifact store's `figure_state` side table since the artifact-authority migration, and reconstructing an edit-history line for model text is deferred to a future Web-row-only surface.
