...
An admitted artifact-viewer edit is one ordinary user turn. Its text lists an ordered, non-empty set of targets, each target's logical artifact and exact version, one instruction, and the requirement to use each version as the corresponding `artifact_inputs` source and `edit_of` parent. The durable `science-edit` source preserves `{ targets, instruction }`; every raster target supplies its exact selected image attachment in target order.
