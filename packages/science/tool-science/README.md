...
The package also owns the `scienceEdits` Typert Remote used by the artifact viewer. Web Hosts mount its `./edit-service` entry in the Host root where the Typert Gateway resolves Remote services; the preset-scoped package root remains the model-facing Consumer and publishes no service. The entry injects both `attachments` (to mint the session-message images `submit` queues below) and `scienceArtifactStore` (to read committed version bytes). `submit` accepts a non-empty ordered array of exact artifact targets plus one instruction. Each target may carry an element comment, validated and trimmed by the same text rules as the instruction. It strictly folds the addressed live Agent's complete session and validates every target's current committed version and its media type before it queues one structured user message; any failure identifies the target position and prevents partial admission. A `normalized-region` target reads its version's bytes from the project artifact store and mints a fresh session-message image per distinct version (repeated region targets against the same version reuse one minted image). `commitStyleEdit` accepts one exact current Vega-Lite parent plus a complete JSON-object spec and appends it directly through `ctx.scienceArtifactStore.appendVersion` as the next contiguous `origin: 'human-edit'` version, with exact ancestry and copied environment provenance but no run fields. A stale selection rejects as `SCIENCE_EDIT_STALE_VERSION`, a media mismatch as `SCIENCE_EDIT_TARGET_MISMATCH`, and an invalid or non-object JSON value as `SCIENCE_EDIT_SPEC_INVALID`; the project artifact store admits a version's bytes with no size cap of its own, so `commitStyleEdit` has no dedicated spec-size bound. Neither method silently substitutes the latest version. The message source preserves `{ kind: 'science-edit', targets, instruction }`, while its text requires the model to use every exact version in the corresponding `artifact_inputs` and `edit_of`; each raster target adds its minted image in target order.
...
An admitted artifact-viewer edit is one ordinary user turn. Its text lists an ordered, non-empty set of targets, each target's logical artifact and exact version plus any element comment, one instruction, and the requirement to use each version as the corresponding `artifact_inputs` source and `edit_of` parent. The durable `science-edit` source preserves `{ targets, instruction }`; every raster target supplies its exact selected image attachment in target order.
...
- **No general chart grammar or Outcome editor** — the viewer can commit bounded style changes to a complete Vega-Lite specification, while the model still owns structural plotting changes and immutable evidence-backed Outcome revisions.
- **`commitStyleEdit` is the first append path concurrent with the agent loop** — a direct style edit can land between two of the model's own turns. A racing auto-capture that computed its next version from a pre-edit fold is rejected loudly by the pre-commit invariant instead of silently overwriting the human edit; the run itself still settles, and the rejected capture degrades to `appendFailed` rather than corrupting the artifact history.
