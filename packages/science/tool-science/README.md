...
The package also owns the `scienceEdits` Typert Remote used by the artifact viewer. Web Hosts mount its `./edit-service` entry in the Host root where the Typert Gateway resolves Remote services; the preset-scoped package root remains the model-facing Consumer and publishes no service. The entry injects both `attachments` and `scienceArtifactStore` so `submit` can read exact committed PNG bytes and mint session-message images. `submit` accepts a non-empty ordered array of normalized raster regions plus one instruction. Each target may carry an element comment, validated and trimmed by the same text rules as the instruction. It strictly folds the addressed live Agent's complete session and validates every target's current committed version and media type before it queues one structured user message; any failure identifies the target position and prevents partial admission. Repeated region targets against the same version reuse one minted image. A stale selection rejects as `SCIENCE_EDIT_STALE_VERSION`, and a media mismatch rejects as `SCIENCE_EDIT_TARGET_MISMATCH`; the method never substitutes the latest version. The message source preserves `{ kind: 'science-edit', targets, instruction }`, while its text requires the model to use every exact version in the corresponding `artifact_inputs` and `edit_of`; each selected raster version adds its minted image in target order.
...
- **No direct chart editor or Outcome editor** — raster-region selections become model instructions; the model owns chart changes and immutable evidence-backed Outcome revisions.
