...
The Host `/read-service` entry exposes seven `science` Remote reads for artifact bytes, library metadata, version summaries, chart state, referenced UTF-8 files, workspace listings, and workspace previews. It also registers authenticated GET/HEAD `/api/science-artifact?sessionId=…&versionId=…` through the connection service. Reads resolve the session’s durable workspace and authorize versions against its Science history or project store. Workspace preview limits are deployment configuration.
