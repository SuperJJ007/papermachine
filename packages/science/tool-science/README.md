...
The package also owns the `scienceEdits` Typert Remote used by the artifact viewer. Web Hosts mount its `./edit-service` entry in the Host root where the Typert Gateway resolves Remote services; the preset-scoped package root remains the model-facing Consumer and publishes no service. `submit` accepts a non-empty ordered array of exact artifact targets plus one instruction. Each target may carry an element comment, validated and trimmed by the same text rules as the instruction. It strictly folds the addressed live Agent's complete session and validates every target's current committed version and attachment media type before it queues one structured user message; any failure identifies the target position and prevents partial admission. `commitStyleEdit` accepts one exact current Vega-Lite parent plus a complete JSON-object spec, admits it through `ctx.attachments.saveText`, and appends the next contiguous `origin: 'human-edit'` version with exact ancestry and copied environment provenance but no run fields. A stale selection rejects as `SCIENCE_EDIT_STALE_VERSION`, a media mismatch as `SCIENCE_EDIT_TARGET_MISMATCH`, and an invalid JSON value as `SCIENCE_EDIT_SPEC_INVALID`; an oversized spec escapes `commitStyleEdit` unwrapped as the attachment store's own `TEXT_TOO_LARGE` from `saveText` admission, not a `SCIENCE_EDIT_*` code. Neither method silently substitutes the latest version. The message source preserves `{ kind: 'science-edit', targets, instruction }`, while its text requires the model to use every exact version in the corresponding `artifact_inputs` and `edit_of`; each raster target adds its exact selected image attachment in target order.

`addArtifactNote` validates an exact committed artifact version, trims plain text, and appends an ignorable user-only note event. `removeArtifactNote` validates an active add-event sequence before appending its ignorable removal. Neither method queues a follow-up or creates model-visible content.
