...
`annotate_artifact` 以文本渲染稳定 artifact id、逻辑名、版本、内容来源、策展状态、标题、可选 caption、媒体类型与字节数；绝不输出文件字节、image content block，也不输出 store 内部 version id、内容校验和或 project id。它与 `run_python`/`run_r` 带标签的客户端 presentation value 都不是模型可见内容——它随 `tool/result.meta` 传递，只由 `dsh-client-ui-science` 的专用行读取。
