...
`addArtifactNote` 校验一个精确的已提交 artifact version，去除纯文本首尾空白，并追加一条可忽略的纯用户备注事件。`removeArtifactNote` 在追加可忽略的删除事件前校验活跃 add-event sequence。两种方法都不排入 follow-up，也不创建 model-visible 内容。
