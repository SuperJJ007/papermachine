...
| `run_python` | `code`（非空字符串）、可选 `artifact_inputs`、可选 `edit_of`、可选 `raster_artifacts` | 通过 `ctx.scienceRuntime.startRun` 在该 session 持久化的 Python kernel 中运行 `code`，并转发该工具调用的取消信号。`artifact_inputs` 把精确的 `{artifactId, version}` 物化到 `SCIENCE_INPUT_DIR` 下的路径；`edit_of` 把捕获相对输出路径映射到精确父版本；`raster_artifacts` 指名这次 run 在 Runtime 默认的 `rasterCapture: 'declared'` 策略下声明要自动捕获的捕获相对 `.png` 路径。其结果会列出本次 run 被自动捕获并持久保存的文件，以及未声明而被跳过的 `.png`（见"Run 结果"）。 |
| `run_r` | `code`（非空字符串）、可选 `artifact_inputs`、可选 `edit_of`、可选 `raster_artifacts` | 对该 session 的持久化 R kernel 应用相同的精确版本 input、edit parent 与 raster 声明行为。 |
