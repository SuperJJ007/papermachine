...
通用的会话附件引用注册表。它拥有 `ctx.sessionAttachments`——唯一实现,把一条持久化会话事件转译为该事件所授权的完整 `ImageAttachmentRef`/`FileAttachmentRef` 值。内置扫描器覆盖当前每一种可能携带图片引用的模型可见内容载体(直接 content、被包裹的助手消息、每条 inserted 消息,以及一个已完成的 `assistant/chunk` 块);领域包则为自己拥有的某个事件类型注册一个带类型的提取器，该提取器可以返回任意一种引用(或两者都返回)。
...
- **只返回完整引用,绝不返回裸 id。** 已注册的提取器校验该事件自身的持久化字段,并返回完整的 `ImageAttachmentRef`/`FileAttachmentRef` 值。一个引用只有出现在该会话自身日志中某个已分类/已注册的载体里才被授权——任意 JSON(工具参数、另一个会话的事件、attachment-free 或未识别的 ignorable 事件)均不授权任何内容。内置载体扫描器(`extractBuiltInAttachments`)仍然只处理图片:目前的模型可见内容块没有文本附件载体类型，因此只有已注册的领域提取器才能返回 `FileAttachmentRef`。
...
本包承担附件授权 seam 中通用注册表的角色:某个领域 host 插件为自己的 extractor-required 事件类型贡献带类型的提取器,
