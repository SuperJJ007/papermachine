...
/**
 * Collect every image or file reference inside one content array, descending into
 ...
 */
function collectFromContent(content: unknown, refs: Array<ImageAttachmentRef | FileAttachmentRef>): void {
  if (!Array.isArray(content)) return
  for (const value of content) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) continue
    const block = value as { type?: unknown; attachment?: unknown; content?: unknown }
    if ((block.type === 'image' || block.type === 'file') && typeof block.attachment === 'object' && block.attachment !== null) {
...
/**
 * Scan one built-in-carrier event for every image or file reference it durably
 ...
 */
export function extractBuiltInAttachments(event: AttachmentCarrierEvent): ReadonlyArray<ImageAttachmentRef | FileAttachmentRef> {
  ...
  if (event.type === 'assistant/attempt' && data.stream !== undefined) {
    for (const record of data.stream) {
      if (record.type === 'chunk' && record.chunk?.type === 'block-end') collectFromContent([record.chunk.block], refs)
    }
  }
  return refs
}
