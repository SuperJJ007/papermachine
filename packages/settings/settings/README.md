...
- `register(ns, schema, { base?, applies? })` — returns the owner `SettingsScope` (`get`/`watch`/`update`). The registration is an effect on the calling plugin's fiber: disposing that fiber removes the namespace and its observers. A stored section the schema rejects fails the registration itself; a duplicate namespace fails loud. `applies` (default `'live'`) is the owner's own effect timing: a `'live'` owner's `scope.get()` and `describe()`'s `effective` both track every commit; a `'restart'` owner reads `scope.get()` once here and never again, so a later write moves `describe()`'s `value` (what is stored) while `effective` (what that owner is still acting on) stays exactly what it read at this call, until the process restarts.
- `describe(options?)` — one descriptor per namespace (`schema.toJSON()` envelope, resolved `value`, `effective` — the value the registered OWNER actually reads, diverging from `value` only for a `restart`-applies namespace after a write — detached `base`/`user` layers, `applies`) for configuration surfaces; a field's presence in `user` is what marks it user-overridden. A configuration UI compares `effective` against `value` to tell a stored change from one the owner has acted on — the fact a restart-scoped settings card needs to show "saved" apart from "in effect". `describe({ redactSecrets: true })` strips `role('secret')` fields from every layer (`value` and `effective` alike) and adds the `secrets` slot list (`{ path, set }`); every wire surface MUST pass it, and the pure `redactSecrets(schema, value)` walker is exported for other wires.
