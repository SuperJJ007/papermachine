...
普通 Linux 启动器在消费请求前被信号终止时，返回实际观察到的信号；请求未消费且无信号的退出仍视为启动失败。
