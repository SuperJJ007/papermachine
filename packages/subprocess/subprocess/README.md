...
- The spec is fully explicit — argv, cwd, per-stream stdio dispositions, grace, and required `environmentBase` (`'scrubbed-parent'` or `'empty'`) — because deployment-varying defaults belong to the caller's config, not to a hidden subprocess-service default (the `dsh-shell` request/spec split is the owning template). `argv` is never shell-interpreted; a consumer that wants a shell passes `['bash', '-c', command]` itself. Existing Consumers name `'scrubbed-parent'` so they keep the historical scrubbed parent base; `'empty'` starts from no inherited names and then applies only `env`.
- `executionWorld` is a readonly provider fact: the local implementation reports `'host-local'`; the E2B implementation reports `'remote'`. A host-path consumer must require `'host-local'` before creating owner records, directories, or Session events.
- Stdio is Node-shaped per stream: `'pipe'` hands the caller the raw stream for its own protocol framing (LSP JSON-RPC, ACP ndjson), `'inherit'` passes the parent descriptor through for diagnostics, and collect mode (`{ maxBytes, spill? }`) buffers a bounded tail with an optional full-stream spill file. Collect readers take whole-stream byte offsets and never consume, so independent readers cannot steal one another's deltas; a read whose offset slid out of the in-memory tail is `lossy` and points at the spill file when one exists. Collected output stays readable after settlement. Each collected read also reports `utf8Validity` for the exact retained byte slice (`'valid'` or `'invalid'` when those bytes are still held; `'unknown'` only when a provider already holds decoded text and has no recoverable bytes). `utf8ValidityOf` is the shared pre-decode check.
...
- `interrupt()` is a separate, narrower verb: a cooperative SIGINT request to the direct child process only, never the tree. It is idempotent, a no-op once the direct child has exited, and a documented no-op on `win32` (no POSIX signal delivery) rather than a throw. Unlike `terminate()`, it makes no escalation or quiescence promise — whether and how the child reacts is between the caller and that child's own cooperation contract.
