...
- spec 完全显式（argv、cwd、按流划分的 stdio 处置方式（disposition）、宽限期，以及必填的 `environmentBase`（`'scrubbed-parent'` 或 `'empty'`）），因为随部署变化的默认值属于调用方的配置，而不属于某个隐藏的子进程服务默认值（`dsh-shell` 的 request/spec 拆分是这条规则的所属模板）。`argv` 绝不经过 shell 解释；需要 shell 的消费方自行传入 `['bash', '-c', command]`。既有消费方显式写出 `'scrubbed-parent'`，以保留历史上的已清理父环境基座；`'empty'` 从不继承任何名称开始，然后只应用 `env`。
- `executionWorld` 是只读的提供方事实：本地实现报告 `'host-local'`；E2B 实现报告 `'remote'`。依赖宿主路径的消费方必须在创建所有者记录、目录或 Session 事件之前要求 `'host-local'`。
- stdio 按流采用 Node 风格：`'pipe'` 把原始流交给调用方做自己的协议分帧（LSP 的 JSON-RPC、ACP（Agent Client Protocol）的 ndjson），`'inherit'` 直通父进程描述符以承载诊断输出，收集模式（collect）`{ maxBytes, spill? }` 则缓冲一段有界尾部，外加可选的完整流 spill 文件。收集模式的读取器接受全流字节偏移量且从不消费，因此独立的读取器不会抢走彼此的增量；偏移量滑出内存尾部窗口的读取标记为 `lossy`，并在 spill 文件存在时指向它。收集到的输出在结算后仍可读取。每次收集读取还会报告所表示精确字节切片的 `utf8Validity`（仍持有这些字节时为 `'valid'` 或 `'invalid'`；仅当提供方已经持有解码文本且无法恢复原始字节时才允许 `'unknown'`）。`utf8ValidityOf` 是共享的解码前检查。
...
- `interrupt()` 是另一个范围更窄的动词：仅向直接子进程发出协作式 SIGINT 请求，从不触及整棵进程树。它是幂等的，在直接子进程退出后即为空操作；在 `win32` 上也有文档记录的空操作行为（没有 POSIX 信号投递），而非抛出异常。与 `terminate()` 不同，它不承诺任何升级或完全停稳：子进程是否以及如何响应，取决于调用方与该子进程自身的协作约定。
