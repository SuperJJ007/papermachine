...
`shellDialect` 选择 shell 栈（默认 `bash`，或 `pwsh`）：它决定默认的 `shellPath`／`shellArgs`（bash 为 `--noprofile --norc -i`；pwsh 经共享的 `dsh-pwsh-local` 解析器得到 `-NoLogo -NoProfile`）与启动契约。bash 方言通过环境安装提示符（`PS1` 加 OSC `133;D;` 终结的 `PROMPT_COMMAND`）。pwsh 无法从环境安装提示符，因此后端先等待初始 shell 提示符，再通过会话写入 `prompt` 函数，并等待完整的受控提示符行——因为 pwsh 从横幅到提示符的间隙可能超过静默上限，所以会在后续 send 上循环等待；同时其环境去掉 bash 专属标记并加 `NO_COLOR`。同一条首发送还会带上共享的 `dsh-pwsh-local` 编码前缀，在一切运行之前把 `[Console]::OutputEncoding` 与 `$OutputEncoding` 钉为 UTF-8：会话解码路径按 UTF-8 读取 PTY 字节，未钉住编码的控制台会以宿主代码页输出非 ASCII 内容。两种方言发出相同的 BEL 终结 OSC 标记，因此就绪机制与消费方与方言无关。 启动的所有后续等待共用一个 `timeoutMs` 时间预算；设置命令的回显不构成就绪证据。

有界的 `@xterm/headless` 屏幕跟踪终端光标状态，并通过同一个子进程终端回答设备查询，使 PowerShell 能在命令输入到达前完成光标位置握手。模拟器使用已配置的行数和列数，不保留滚动历史；模型可见输出仍由现有逐行缓冲区提供。关闭时释放模拟器，协议响应写入失败则使终端传输失败。该机制支持终端协商，不提供全屏 UI。参见 [PowerShell 设备响应决策](../../../.agents/notes/implemented/bug-fix/2026-09-08-pwsh-terminal-device-responses.zh.md)。
