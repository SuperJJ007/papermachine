...
describe('runScenario', () => {
  ...
  it('waitForInboxMessage times out when the session log or matching insertion is absent', { timeout: 20_000 }, async () => {
    ...
    await expect(runScenario(
      { steps: [...boot, { op: 'promptAndCancel', text: 'hang' }, { op: 'waitForInboxMessage', text: 'missing', timeoutMs: 1000 }] },
      { agent: AGENT, mode: 'replay', fixtureFile: absent.fixtureFile },
    )).rejects.toThrow(/did not persist expected inbox message within 1000ms/)
  ...
  it('waitForTurnStart rejects missing, earlier, and malformed durable turns', { timeout: 20_000 }, async () => {
    ...
    await expect(runScenario(
      { steps: [...boot, { op: 'waitForTurnStart', timeoutMs: 1000 }] },
      { agent: AGENT, mode: 'replay', fixtureFile: missing.fixtureFile },
    )).rejects.toThrow(/did not persist turn\/start within 1000ms/)
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForTurnStart', minimumTurn: 3, timeoutMs: 1000 },
        ],
      },
      { agent: AGENT, mode: 'replay', fixtureFile: earlier.fixtureFile },
    )).rejects.toThrow(/turn\/start at or beyond turn 3 within 1000ms/)
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForTurnStart', timeoutMs: 1000 },
        ],
      },
  ...
  it('waitForTurnEnd times out for a missing log and an open logged turn', { timeout: 20_000 }, async () => {
    const missing = await scenario({})
    await expect(runScenario(
      { steps: [...boot, { op: 'waitForTurnEnd', timeoutMs: 1000 }] },
      { agent: AGENT, mode: 'replay', fixtureFile: missing.fixtureFile },
    )).rejects.toThrow(/did not persist turn\/end within 1000ms/)
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForTurnEnd', timeoutMs: 1000 },
        ],
      },
  ...
  it('waitForGoalPhase requires the requested durable goal phase', { timeout: 20_000 }, async () => {
    ...
    const missing = await scenario({})
    await expect(runScenario(
      { steps: [...boot, { op: 'waitForGoalPhase', phase: 'blocked', timeoutMs: 1000 }] },
      { agent: AGENT, mode: 'replay', fixtureFile: missing.fixtureFile },
    )).rejects.toThrow(/did not persist goal phase "blocked" within 1000ms/)
  })
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForSubagentTurnEnd', minimumTurn: 2, timeoutMs: 1000 },
        ],
      },
      { agent: AGENT, mode: 'replay', fixtureFile: closed.fixtureFile },
    )).rejects.toThrow(/subagent child #1 did not persist closed turn 2 within 1000ms/)
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForSubagentTurnEnd', timeoutMs: 1000 },
        ],
      },
      { agent: AGENT, mode: 'replay', fixtureFile: seedOnly.fixtureFile },
    )).rejects.toThrow(/subagent child #1 did not persist closed turn 1 within 1000ms/)

    const missing = await scenario({})
    await expect(runScenario(
      { steps: [...boot, { op: 'waitForSubagentTurnEnd', child: 2, timeoutMs: 1000 }] },
      { agent: AGENT, mode: 'replay', fixtureFile: missing.fixtureFile },
    )).rejects.toThrow(/subagent child #2 did not persist closed turn 1 within 1000ms/)
  })
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForTitleAfterTurnEnd', timeoutMs: 1000 },
        ],
      },
      { agent: AGENT, mode: 'replay', fixtureFile },
    )).rejects.toThrow(/did not persist session\/title after turn\/end within 1000ms/)
  })
    ...
    await expect(runScenario(
      {
        steps: [
          ...boot,
          { op: 'promptAndCancel', text: 'hang' },
          { op: 'waitForEventAfterTurnEnd', type: 'user/message', timeoutMs: 1000 },
        ],
      },
      { agent: AGENT, mode: 'replay', fixtureFile: early.fixtureFile },
    )).rejects.toThrow(/did not persist user\/message after turn\/end within 1000ms/)
  })
