...
describe('fixture session face', () => {
  it('fail-loud stubs name the missing verb; supplied overrides run instead', async () => {
    ...
    expect(() => bare.readTextAttachment('att-1' as Parameters<typeof bare.readTextAttachment>[0])).toThrow(/readTextAttachment is not stubbed/)
    expect(() => bare.readScienceArtifact('version-1' as Parameters<typeof bare.readScienceArtifact>[0])).toThrow(/readScienceArtifact is not stubbed/)
    expect(() => bare.readScienceLibrary()).toThrow(/readScienceLibrary is not stubbed/)
    expect(() => bare.readScienceVersions(['version-1' as never])).toThrow(/readScienceVersions is not stubbed/)
    expect(() => bare.readScienceChartState('version-1' as Parameters<typeof bare.readScienceChartState>[0])).toThrow(/readScienceChartState is not stubbed/)
    expect(() => bare.readWorkspaceFiles()).toThrow(/readWorkspaceFiles is not stubbed/)
    expect(() => bare.readWorkspaceFile('result.csv')).toThrow(/readWorkspaceFile is not stubbed/)
