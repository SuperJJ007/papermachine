...
The production deployment uses pnpm’s shared lockfile to create a standalone hoisted installation inside the carrier directory. It preserves source workspace dependencies and resolves the complete runtime closure to one Cordis instance. A missing or incomplete shared lockfile fails before deployment; legacy deployment is disabled. Peer installation follows the recorded lockfile settings.

Unused workspace patches are permitted because the closure excludes desktop packaging tools. A temporary pnpm configuration hook translates local-package build approvals and denials to the file-URL identities used by deployment; unreviewed scripts still fail. The hook is removed on success or failure. Every pnpm invocation owned by the builder disables automatic dependency refresh, including the closure preflight, build, deployment and pkg steps. The closure check and explicit deployment still run; development tools must already be installed.
